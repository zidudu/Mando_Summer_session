...
- 출력(매 50ms 동기 퍼블리시):
    ...
    /gps/wp_index     (Int32, 1-based "타겟 인덱스" 퍼블리시)
    /gps/speed_cmd    (Float32)
    /gps/GRADEUP_ON   (Int32, 0/1)
    /gps/rtk_status   (String, "FIX"/"NONE")
- 특징:
    • 50ms 고정 주기 타이머에서 일괄 계산/퍼블리시
    • STOP_INDEX(예: 14) 타겟 원 '최초 진입' 시 4초 정지 라치
    • 정지/저속/hold 중엔 '이동 중 얻은 마지막 헤딩 스냅샷' 사용
    • 재출발 시(hold 해제 직후) 2초 동안만 조향 0 강제 (헤딩 계산은 계속 유지)

[중요 변경점]
- 이전: 현재 위치가 어떤 WP 원 안에 있는지 계산해 /gps/wp_index 퍼블리시 (반경 밖=0)
- 현재: "항상 타겟 인덱스(1-based)"를 /gps/wp_index 로 퍼블리시 (반경 여부와 무관)
- Grade(접근/정지/탈출) 판정도 '현재 위치 인덱스'가 아니라 '타겟 인덱스'로만 수행
"""
...
TOPIC_WP_INDEX      = '/gps/wp_index'        # ← 이제 "타겟 인덱스(1-based)" 의미로 퍼블리시
...
GRADE_APPROACH_START = 11
GRADE_APPROACH_END   = 13
GRADE_STOP_INDEX     = 14   # ← 이 원 '최초 진입' 시 4초 정지 라치
GRADE_EXIT_START     = 16
GRADE_EXIT_END       = 17
GRADE_HOLD_SEC       = 4.0
...
nav_target_idx0 = 0                  # 0-based 타겟 인덱스(항상 진행용 기준)
...
# ── LPF 전역(각 루프에서 갱신) ───────────────────────────
ALPHA = 0.0               # DT/(TAU+DT), on_timer에서 매 틱 갱신
filtered_steering_angle = 0.0  # LPF 내부 상태(유지)
...
waypoint_csv_default = os.path.join(pkg_path, 'config', 'raw_track_latlon_19.csv')
...
def _grade_logic_by_target(target_1b, steer_cmd):
    """
    타겟 인덱스(1-based)만을 사용해 Grade On/Off 및 속도/조향/정지 라치 결정.
    반환: (steer_out, speed_cmd, grade_val)
    ...
    # 접근/진행/탈출 구간에 따른 grade 토픽
    if target_1b is not None:
        if GRADE_APPROACH_START <= target_1b <= GRADE_APPROACH_END:
            grade_val = 1
        elif GRADE_EXIT_START <= target_1b <= GRADE_EXIT_END:
            grade_val = 0
        # 필요 시 추가 구간(예: 7~8 유지)을 넣을 수 있음
...
def on_timer(_evt):
    ...
    rospy.loginfo(
        "dt=%.3f | steer=%.2f°, speed=%.2f, grade=%d, tgt_idx=%d | hold_active=%s hold_done=%s restart_zero=%s",
        dt, steer_out, speed_cmd, grade_val, target_1b,
...
def update_plot(_):
    ...
    ax.set_title('50 ms timer · τ=0.25 s; limit ±30° (wp_index=target 1-based)')

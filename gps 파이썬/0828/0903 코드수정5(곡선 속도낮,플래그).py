...
[수정됨]
7) 곡률 기반 속도 제어: 경로의 곡률을 미리 계산하여 커브 진입 시 자동 감속
8) 웨이포인트 액션 플래그: CSV의 'Action' 플래그를 읽어 특정 지점 통과 시 지정된 동작 수행
...
# ── 전역 기본값 ─────────────────────────────────────
# 작은 차에 맞춰 기본값 수정
TARGET_RADIUS_DEFAULT = 0.2
MIN_WAYPOINT_DISTANCE_DEFAULT = 0.5
FC_DEFAULT = 2.0              # steer LPF [Hz]
FS_DEFAULT = 20.0             # loop [Hz]
STEER_LIMIT_DEG_DEFAULT = 25.0  # 조향 명령 제한 [deg] (작은 차는 더 클 수 있음)

# 속도 명령 기본/상한
SPEED_MODE_DEFAULT     = "curve" # [수정] 기본 속도 모드를 'curve'로 변경
SPEED_CAP_MPS_DEFAULT  = 1.4     # 5km/h

# [신규] 곡률 기반 속도 제어 파라미터 기본값
BASE_SPEED_MPS_DEFAULT = 1.2
MIN_SPEED_MPS_DEFAULT = 0.5
CURVE_LOOKAHEAD_IDX_DEFAULT = 3

# ✅ 하드 고정(원하면 사용) - curve 모드 사용 시 비활성화 권장
# SPEED_FORCE = ('const', 1.0)
SPEED_FORCE = None

# (1) 헤딩 안정화 파라미터
HEADING_MIN_SPEED_DEFAULT = 0.20  # [수정] 저속 기준 하향
HEADING_ALPHA_DEFAULT     = 0.25  # 원형 EMA 계수(0~1)
BOOTSTRAP_DIST_DEFAULT    = 1.0   # [수정] 시작 직후 전진거리
...
RESTART_MIN_SPEED_DEFAULT= 0.20   # [m/s] 재출발 간주

# (C) 조향 데드존(이동 변화율 기반)
STEER_DEADZONE_MOVE_RATIO_DEFAULT = 0.005 # dist_to_target의 1% 미만 이동이면
STEER_DEADZONE_MOVE_MIN_M_DEFAULT = 0.01  # 최소 1cm 기준
...
RECOVERY_SEARCH_RADIUS_DEF     = 5.0   # [수정] [m] 복귀 후보 검색 반경
RECOVERY_LOOKAHEAD_N_DEF       = 20    # [수정] 앞쪽 인덱스 범위
...
# [수정] 웨이포인트 관련 전역 변수 추가
waypoints_x = None
waypoints_y = None
waypoints_curvature = None
waypoints_actions = None
_current_action_zone = "NONE" # 현재 활성화된 액션 존 이름
waypoint_index = 0
...
def calculate_curvature(p_prev, p_curr, p_next):
    """세 점을 이용해 곡률(turn angle)을 계산하는 함수"""
    vec_a = np.array(p_curr) - np.array(p_prev)
    vec_b = np.array(p_next) - np.array(p_curr)
    ...
    cos_theta = clamp(cos_theta, -1.0, 1.0) # 수치 오류 방지

    # 턴 각도를 degree로 반환 (0:직선, 값이 클수록 급커브)
    angle_rad = math.acos(cos_theta)
    return math.degrees(angle_rad)


# [신규] 특별 동작 실행 함수
def execute_special_action(action_name):
    """특정 웨이포인트에 도달했을 때 실행될 함수"""
    rospy.loginfo(f"======== ACTION TRIGGERED: {action_name} ========")
    # 추후 이 함수 내부에 실제 동작을 구현합니다.
    # 예: if action_name == "SPRAY_START": ...
    pass
...
def load_waypoints(path_csv, min_wp_dist):
    # [수정] 전역 변수 사용 선언
    global waypoints_curvature, waypoints_actions

    df = pd.read_csv(path_csv)
    # [수정] Action 열이 없으면 빈 값으로 채우기
    if 'Action' not in df.columns:
        df['Action'] = ""
    df['Action'] = df['Action'].fillna("") # NaN 값을 빈 문자열로

    coords = [latlon_to_meters(row['Lat'], row['Lon']) for _, row in df.iterrows()]
    actions = list(df['Action']) # Action 리스트 추출

    if len(coords) < 1:
        raise RuntimeError("waypoints csv empty")

    # [수정] Action 정보도 함께 필터링
    fx, fy, fa = [float(coords[0][0])], [float(coords[0][1])], [actions[0]]
    for i in range(1, len(coords)):
        xi, yi = coords[i]
        if distance_m(fx[-1], fy[-1], xi, yi) >= min_wp_dist:
            fx.append(float(xi))
            fy.append(float(yi))
            fa.append(actions[i]) # 필터링된 인덱스에 맞춰 Action 추가

    # [수정] 곡률 계산 로직 추가
    curvatures = [0.0] * len(fx)
    if len(fx) >= 3:
        for i in range(1, len(fx) - 1):
            p_prev = (fx[i-1], fy[i-1])
            p_curr = (fx[i],   fy[i])
            p_next = (fx[i+1], fy[i+1])
            curvatures[i] = calculate_curvature(p_prev, p_curr, p_next)
    
    waypoints_curvature = np.array(curvatures)
    waypoints_actions = fa
    rospy.loginfo(f"곡률 계산 완료. 최대 곡률: {np.max(waypoints_curvature):.2f} deg")
...
def compute_speed_command():
    ...
    반환:
      ...
      mode_str   : "curve" | "const" | "code(force)" | "const(force)"
    """
    global waypoint_index, waypoints_curvature, _in_recovery, _recovery_target_index
    ...
    if mode == "curve":
        ...
        base_speed = float(params.get('base_speed_mps', BASE_SPEED_MPS_DEFAULT))
        min_speed = float(params.get('min_speed_mps', MIN_SPEED_MPS_DEFAULT))

        # 턴 각도가 5도 이하면 감속 없음, 45도 이상이면 최대 감속(최소 속도)
        turn_angle_for_calc = clamp(target_curvature, 5.0, 45.0)
        speed_ratio = 1.0 - ((turn_angle_for_calc - 5.0) / (45.0 - 5.0))
        
        mps = min_speed + (base_speed - min_speed) * speed_ratio
        capm = float(params.get('speed_cap_mps', SPEED_CAP_MPS_DEFAULT))
...
def update_plot_once(ax):
    ...
    global _in_recovery, _recovery_target_index, _radius_hit_consec, _current_action_zone
    ...
    for i in range(start, end):
        ...
        # [수정] Action이 있는 웨이포인트에 텍스트 표시
        if waypoints_actions and waypoints_actions[i]:
            ax.text(waypoints_x[i], waypoints_y[i] + 0.5, waypoints_actions[i],
                    color='green', fontsize=8, ha='center', style='italic')
    ...
    if cx and cy:
        ...
        # [수정] 정상 모드에서만 인덱스 증가 및 Action 실행
        if (not _in_recovery) and (_radius_hit_consec >= int(params['radius_hit_count'])):
            # Action 플래그 확인
            if waypoint_index < len(waypoints_actions):
                action_name = waypoints_actions[waypoint_index]
                if action_name: # Action 이름이 비어있지 않다면
                    _current_action_zone = action_name
                    execute_special_action(action_name)
        ...
        if now - _last_log_t > 0.5:
            ...
            # [수정] 속도 표시 포맷
            speed_report_val = _last_speed_cmd
            if _last_speed_mode == "curve":
                speed_report_val = f"{_last_speed_cmd:.2f} m/s"

            rospy.loginfo(f"{latlon_txt}, SpeedCmd({_last_speed_mode})={speed_report_val}, v(EMA)={v_meas:.2f} m/s, "
        ...
        speed_report_str = _last_speed_cmd
        if isinstance(_last_speed_cmd, float):
            speed_report_str = f"{_last_speed_cmd:.2f}"
        info_lines.append(f"SpeedCmd({_last_speed_mode}): {speed_report_str}")
        info_lines.append(f"Action Zone: {_current_action_zone}") # 현재 액션 존 표시
...
def main():
    ...
    params = {
        ...
        # [신규] 곡률 기반 속도 제어 파라미터
        'base_speed_mps':      float(rospy.get_param('~base_speed_mps', BASE_SPEED_MPS_DEFAULT)),
        'min_speed_mps':       float(rospy.get_param('~min_speed_mps', MIN_SPEED_MPS_DEFAULT)),
        'curve_lookahead_idx': int(rospy.get_param('~curve_lookahead_idx', CURVE_LOOKAHEAD_IDX_DEFAULT)),

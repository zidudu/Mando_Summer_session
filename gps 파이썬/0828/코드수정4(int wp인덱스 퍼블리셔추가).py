...
- 퍼블리시: /vehicle/speed_cmd (Float32), /vehicle/steer_cmd (Float32, deg), /rtk/status (String)
           /current_waypoint_idx (Int32)  # 추가된 부분
...
import rospy
import rospkg
from std_msgs.msg import Float32, String, Int32  # Int32 추가
...
# (C) 조향 데드존(이동 변화율 기반)
STEER_DEADZONE_MOVE_RATIO_DEFAULT = 0.01  # dist_to_target의 1% 미만 이동이면
STEER_DEADZONE_MOVE_MIN_M_DEFAULT = 0.05  # 최소 5cm 기준
...
pub_steer = None
pub_rtk   = None
pub_waypoint_idx = None  # 추가된 부분: 웨이포인트 인덱스를 퍼블리시하는 퍼블리셔
...
# 추가된 부분: current_waypoint_idx 퍼블리시
def publish_waypoint_idx(idx):
    if pub_waypoint_idx:
        pub_waypoint_idx.publish(Int32(data=idx))
...
# ── 메인 ────────────────────────────────────────────
def main():
    global pub_speed, pub_steer, pub_rtk, pub_waypoint_idx, waypoints_x, waypoints_y, alpha, params
    ...
    # 퍼블리셔
    global pub_speed, pub_steer, pub_rtk, pub_waypoint_idx
    ...
    pub_waypoint_idx = rospy.Publisher('/current_waypoint_idx', Int32, queue_size=10)  # 웨이포인트 인덱스 퍼블리셔 추가

...
기능 요약:
1) 시작 워밍업: start_warmup_sec 동안 speed=1, steer=0 고정
2) 정지-재출발 안정화: 스냅샷/버퍼 기반 헤딩 유지 + 이동 데드존 시 steer=0
3) 순차 인덱스 + 반경 히스테리시스(radius_hit_count회 연속 진입 시 인덱스 증가)
4) 전/후 LPF(좌표 게이팅+LPF, 조향 LPF, 헤딩 원형 EMA)
5) 속도 명령: code/const + 상한 + 하드고정(SPEED_FORCE)
6) 복귀 모드(RECOVER): 경로 이탈 시, 헤딩 ±각/거리/앞쪽 조건을 만족하는 웨이포인트로 복귀
   → 복귀 타겟 반경 진입 시 TRACK으로 전환 및 waypoint_index 동기화
"""
...
# ✅ 하드 고정(필요 시 사용: None이면 비활성)
SPEED_FORCE = ('code', 1)
# SPEED_FORCE = None

# 헤딩 안정화
HEADING_MIN_SPEED_DEFAULT = 0.35   # [m/s]
HEADING_ALPHA_DEFAULT     = 0.25
BOOTSTRAP_DIST_DEFAULT    = 2.0    # [m]

# 시작 워밍업
START_WARMUP_SEC_DEFAULT = 2.0     # [s]

# 정지/재출발
STOP_SPEED_EPS_DEFAULT   = 0.05    # [m/s]
STOP_HOLD_SEC_DEFAULT    = 0.5     # [s]
RESTART_MIN_SPEED_DEFAULT= 0.30    # [m/s]

# 조향 데드존(이동 변화율)
STEER_DEADZONE_MOVE_RATIO_DEFAULT = 0.01
STEER_DEADZONE_MOVE_MIN_M_DEFAULT = 0.05

# 헤딩 후보 선택
HEAD_BUF_LEN_DEFAULT     = 5
HEAD_SELECT_MAX_DEG_DEF  = 90.0
HEAD_GOOD_RATIO_DEFAULT  = 0.75

# 반경 히스테리시스
RADIUS_HIT_COUNT_DEFAULT = 2

# ── 복귀(Rejoin) 파라미터 ──────────────────────────
REJOIN_ENABLE_DEFAULT       = True
REJOIN_TRIGGER_MULT_DEFAULT = 2.0   # trigger_dist = mult * target_radius
REJOIN_SEARCH_RADIUS_DEF    = 15.0  # [m]
REJOIN_MAX_ANGLE_DEG_DEF    = 30.0  # [deg] 현재 헤딩 기준
REJOIN_WINDOW_BACK_DEF      = 5     # 최근접 인덱스에서 뒤로 탐색
REJOIN_WINDOW_AHEAD_DEF     = 80    # 최근접 인덱스에서 앞으로 탐색
REJOIN_MIN_ADVANCE_DEF      = 1     # 현재 waypoint_index보다 최소 몇 칸 앞
REJOIN_EXIT_MULT_DEF        = 1.0   # exit when dist_to_rejoin <= mult*target_radius
...
# 모드/복귀상태
_MODE = "TRACK"                # "TRACK" | "RECOVER"
_rejoin_target_idx = None      # RECOVER 타겟 인덱스
_last_nearest_idx = 0          # 최근 최근접 인덱스 기록(디버그/가속용)
...
def nearest_waypoint_index(x, y):
    """가장 가까운 웨이포인트 인덱스와 거리 반환"""
    dx = waypoints_x - float(x)
    dy = waypoints_y - float(y)
    d2 = dx*dx + dy*dy
...
# ── 복귀 후보 탐색 ──────────────────────────────────
def find_rejoin_target(cx, cy, heading_rad, nearest_idx):
    """
    현재 위치(cx,cy), 헤딩, 최근접 인덱스 기준으로
    - forward(현재 인덱스보다 앞) + 거리 <= search_radius
    - 헤딩과의 각도 <= max_angle
    조건을 만족하는 웨이포인트 중 '가깝고 각오차 작은' 후보를 선택
    """
    N = len(waypoints_x)
    if heading_rad is None:
        # 헤딩이 없으면 기본 진행 방향 x+
    ...
    start_k = max(0, nearest_idx - int(params['rejoin_window_back']))
    end_k   = min(N, nearest_idx + int(params['rejoin_window_ahead']) + 1)

    min_idx_required = min(N-1, waypoint_index + int(params['rejoin_min_advance']))

    best_k = None
    best_score = 1e18

    for k in range(start_k, end_k):
        if k < min_idx_required:
            continue  # 반드시 현재 index보다 앞

        tx, ty = waypoints_x[k], waypoints_y[k]
        vx, vy = tx - cx, ty - cy
        dist = math.hypot(vx, vy)
        if dist > float(params['rejoin_search_radius']):
            continue
        ...
        ang_deg = abs(math.degrees(math.acos(cosang)))

        if ang_deg > float(params['rejoin_max_angle_deg']):
            continue

        # 점수가 낮을수록 좋음: 거리 + 각도 가중
        score = dist + 0.1 * ang_deg
        if score < best_score:
            best_score = score
            best_k = k
...
def update_plot_once(ax):
    ...
    global _head_buf, _MODE, _rejoin_target_idx, _last_nearest_idx
    ...
    with _state_lock:
        ...
        mode_now = _MODE
        rejoin_idx_now = _rejoin_target_idx
    ...
    if cx and cy:
        ...
        # ── 현재 타겟 인덱스 결정 (모드별) ──
        if mode_now == "RECOVER" and rejoin_idx_now is not None:
            active_index = rejoin_idx_now
        else:
        ...
        if warmup_active:
            ...
            if (raw_heading is None) or use_zero_steer or stopped_now:
                ...
                if use_zero_steer: flags.append("DEADZONE")
                if stopped_now:    flags.append("STOP_HOLD")
            else:
        ...
        # ── RECOVER 모드: 복귀 타겟 진입 시 TRACK 복귀 ──
        if mode_now == "RECOVER" and rejoin_idx_now is not None:
            exit_dist = float(params['rejoin_exit_mult']) * float(params['target_radius'])
            if distance_m(cx[-1], cy[-1], tx, ty) <= exit_dist:
                # 동기화 후 TRACK 전환
                waypoint_index = max(waypoint_index, rejoin_idx_now)
                _MODE = "TRACK"
                _rejoin_target_idx = None
                flags.append("REJOIN_DONE")

        # ── (모드 전환) TRACK에서 이탈 감지 → RECOVER 진입 ──
        if bool(params['rejoin_enable']) and _MODE == "TRACK":
            # 최근접 웨이포인트와 거리
            jnear, dnear = nearest_waypoint_index(cx[-1], cy[-1])
            _last_nearest_idx = jnear
            trigger_dist = float(params['rejoin_trigger_mult']) * float(params['target_radius'])

            if dnear > trigger_dist:
                # 후보 탐색
                k = find_rejoin_target(cx[-1], cy[-1], heading_rad, jnear)
                if k is not None:
                    _MODE = "RECOVER"
                    _rejoin_target_idx = k
                    flags.append(f"REJOIN->k={k+1}")
                else:
                    flags.append("REJOIN_NO_CAND")
                ...
                with open(params['log_csv'], 'a', newline='') as f:
                    ...
                    if new:
                        w.writerow([
                            ...
                            'mode','waypoint_idx','target_x','target_y',
                    ...
                    w.writerow([
                        cx[-1], cy[-1], px, py,
                        _MODE, active_index+1, tx, ty,
                        smooth_deg, heading_deg,
                        _last_speed_cmd, v_meas,
                        distance_m(cx[-1], cy[-1], tx, ty), log_time, rtk_txt, _last_speed_mode, "|".join(flags)
                    ])
        ...
        if now - _last_log_t > 0.5:
            ...
            rospy.loginfo(f"{latlon_txt}, Mode={_MODE}, Target={active_index+1}, "
                          f"SpeedCmd({_last_speed_mode})={_last_speed_cmd}, v(EMA)={v_meas:.2f} m/s, "
                          f"Steer={smooth_deg:+.2f} deg, Heading={heading_deg:.2f} deg, "
                          f"RTK={rtk_txt}, Flags={','.join(flags)}")
            _last_log_t = now

        # Info 박스
        info_lines.append(f"Mode: {_MODE}")
        info_lines.append(f"Target #{active_index+1}")
...
def main():
    ...
    params = {
        ...
        # 복귀(Rejoin)
        'rejoin_enable':        bool(rospy.get_param('~rejoin_enable', REJOIN_ENABLE_DEFAULT)),
        'rejoin_trigger_mult':  float(rospy.get_param('~rejoin_trigger_mult', REJOIN_TRIGGER_MULT_DEFAULT)),
        'rejoin_search_radius': float(rospy.get_param('~rejoin_search_radius', REJOIN_SEARCH_RADIUS_DEF)),
        'rejoin_max_angle_deg': float(rospy.get_param('~rejoin_max_angle_deg', REJOIN_MAX_ANGLE_DEG_DEF)),
        'rejoin_window_back':   int(rospy.get_param('~rejoin_window_back', REJOIN_WINDOW_BACK_DEF)),
        'rejoin_window_ahead':  int(rospy.get_param('~rejoin_window_ahead', REJOIN_WINDOW_AHEAD_DEF)),
        'rejoin_min_advance':   int(rospy.get_param('~rejoin_min_advance', REJOIN_MIN_ADVANCE_DEF)),
        'rejoin_exit_mult':     float(rospy.get_param('~rejoin_exit_mult', REJOIN_EXIT_MULT_DEF)),

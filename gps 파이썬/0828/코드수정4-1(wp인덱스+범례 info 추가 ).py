...
def update_plot_once(ax):
    ...
    if bool(params['show_all_waypoints']):
        ...
        for i in range(len(waypoints_x)):
            ...
            ax.add_patch(c)
            ax.text(waypoints_x[i], waypoints_y[i], f"{i+1}", fontsize=7, ha='center', label=f"WP {i+1}")  # 인덱스를 label로 추가
    else:
    ...
    if cx and cy:
        ...
        # Info 박스
        info_lines.append(f"Current Waypoint Index: {waypoint_index+1}")  # 현재 웨이포인트 인덱스

...
입출력:
- 구독: /ublox/fix (sensor_msgs/NavSatFix)
       (옵션) /ublox/navrelposned, /ublox/navpvt
- 퍼블리시: /vehicle/speed_cmd (Float32, m/s), /vehicle/steer_cmd (Float32, deg), /rtk/status (String)

개선(요청 1·2·4 반영):
1) 헤딩 안정화 4종
   - bootstrap_dist 전에는 헤딩 갱신 금지
   - heading_min_speed 미만에서는 헤딩 동결(hold-last)
   - 원형 EMA(각도 래핑 고려)로 헤딩 스무딩
   - “마지막 확정 헤딩 벡터”로 조향각 계산(정지/저속 시에도 안정)
2) 동적 이상치 게이팅 + 스텝 클램프
   - max_step = max(floor_m, k_vel*v_ema*dt + k_sigma*sigma_xy + rtk_margin)
   - 초과분은 방향 유지하며 길이만 클램프
4) 시각화 축 고정
   - 웨이포인트 extents+margin으로 x/y축 고정(짤림 방지)
"""
...
def _cb_navpvt(msg):
    # 3번(센서 headMot 병합)은 제외(요청에 따라 미사용)
    pass

def _cb_fix(msg: NavSatFix):
    """GNSS 좌표 수신 → 동적 게이팅+LPF → 속도/헤딩 갱신(임계속도/부트스트랩 고려)"""
...
def steering_from_vectors(heading_rad, cx, cy, tx, ty):
    ...
    ang = math.degrees(math.atan2(det, dot))   # 부호 있는 각
...
def update_plot_once(ax):
    ...
    if cx and cy:
                ...
                with open(params['log_csv'], 'a', newline='') as f:
                    ...
                    if len(cx) > 1:
                        w.writerow([cx[-1], cy[-1], cx[-2], cy[-2],
                                    ...
                                    speed_show, dist_to_target, log_time, rtk_txt])
                    else:
                        w.writerow([cx[-1], cy[-1], '', '',
                                    tx, ty, smooth_deg, heading_deg,
                                    speed_show, distance_m(cx[-1], cy[-1], tx, ty), log_time, rtk_txt])
        ...
        if now - _last_log_t > 0.5:
            ...
            dist_to_target = distance_m(cx[-1], cy[-1], tx, ty)
            rospy.loginfo(f"{latlon_txt}, v(EMA): {speed_show:.2f} m/s, "

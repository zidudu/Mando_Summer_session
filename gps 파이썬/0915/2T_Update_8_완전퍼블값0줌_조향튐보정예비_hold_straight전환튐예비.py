...
# 0917 추가
# 전역 토글 (기본: 안전모드 유지)
GRADE_TRANSITION_STRICT_STOP = False  # True: 전환 틱에도 0 유지(안전). False: 전환 즉시 STRAIGHT 속도 사용.
def grade_transition_force_on_hold_exit(now):
    """
    HOLD에서 STRAIGHT로 전환할 때 어떤 강제값을 반환할지 결정하는 헬퍼.
    호출 시점: grade_stop_update() 내에서 HOLD 타이밍을 넘긴 직후.
    반환: (v_force, steer_force, grade_flag) -- None인 항목은 '변경 없음' 의미.
    """
    # 안전 우선: 전환 틱에도 0을 유지
    if GRADE_TRANSITION_STRICT_STOP:
        return (0, 0.0, 1)
    # 즉시 출발 모드: STRAIGHT 속도를 바로 반환
    return (GRADE_STRAIGHT_SPEED, 0.0, 1)
# 0917 추가
# ========== (A) 전역 토글/버퍼 초기화: 전역 상태 블록 바로 아래에 넣으세요 ==========
STEER_BOUNCE_ON = False  # True로 켜면 튐 보정 로직 동작
# 헤딩 후보 저장용(간단 초기화)
_stop_heading_snapshot = None     # 정지 시 저장해 둘 스냅샷 헤딩 (rad)
_last_fix_heading_rad = None      # 마지막 확정 헤딩 (rad)
from collections import deque
_head_buf = deque(maxlen=6)       # (heading_rad, timestamp, misc) 형태를 담는 버퍼
# ========== (B) 헬퍼: 각도 차 계산 (유틸 섹션 이후에 넣으세요) ==========
def ang_diff_deg(a_deg, b_deg):
    """두 각도(도) 차의 절대값(0..180) 반환"""
    d = (a_deg - b_deg + 180.0) % 360.0 - 180.0
    return d
# ========== (C) 간단화된 안정화 함수 (유틸 섹션 이후에 넣으세요) ==========
def select_stable_heading_simple(bearing_to_target_deg, raw_heading_deg):
    """
    최소 기능: 후보들 중 target 베어링과 오차가 가장 작은 것을 고름.
    반환값: 선택된 헤딩(deg). 후보가 없으면 raw_heading_deg 반환.
    전역 토글 STEER_BOUNCE_ON 을 꺼두면 아무것도 안 함.
    """
    global _stop_heading_snapshot, _last_fix_heading_rad, _head_buf, STEER_BOUNCE_ON

    if not STEER_BOUNCE_ON:
        return raw_heading_deg

    # 상수(필요시 조정)
    HEAD_SELECT_MAX_DEG = 20.0
    HEAD_GOOD_RATIO = 0.75

    # 후보 수집
    cand = []
    if _stop_heading_snapshot is not None:
        cand.append(math.degrees(_stop_heading_snapshot))
    if _last_fix_heading_rad is not None:
        cand.append(math.degrees(_last_fix_heading_rad))
    if _head_buf:
        seen = set()
        for item in list(_head_buf):
            h = item[0]
            k = round(h, 6)
            if k not in seen:
                cand.append(math.degrees(h))
                seen.add(k)
    ...
    best_err = 1e9
    for h_deg in cand:
        err = abs(ang_diff_deg(bearing_to_target_deg, h_deg))
        if err <= HEAD_SELECT_MAX_DEG and err <= HEAD_GOOD_RATIO * raw_err:
            if err < best_err:
                best_err = err
                best_h = h_deg
    ...
    if _last_fix_heading_rad is not None:
        return math.degrees(_last_fix_heading_rad)
    return raw_heading_deg
...
def grade_stop_update(now):
    ...
    if grade_stop_state == STATE_HOLD:
        if now >= grade_stop_t_end:
            # 0917 추가
            # 전환 시 반환값을 헬퍼에 맡김(false이면 0,0,1임)
            v_force, steer_force, grade_flag = grade_transition_force_on_hold_exit(now)
            ...
            return (True, v_force, steer_force, grade_flag)  # 0917 추가. 기존은 0, 0, 1
        else:
            return (True, 0, 0.0, 1)
...
def _on_shutdown():
    ...
    try:
        # 0917 추가
         # 타이머 먼저 멈춰서 publish_all이 뒤늦게 또 쏘지 못하게
        if pub_timer is not None:
            pub_timer.shutdown()
         # 0을 두세 번 보낸 뒤 짧게 대기(전송 여유)
        for _ in range(2): # 0917 추가
            if pub_speed: pub_speed.publish(Float32(0.0))
            if pub_steer: pub_steer.publish(Float32(0.0))
            if pub_wpidx: pub_wpidx.publish(Int32(0))
            if pub_grade: pub_grade.publish(Int32(0))
            if pub_rtk:   pub_rtk.publish(String("NONE"))
            rospy.sleep(0.03) # 0917 추가
...
def main():
    ...
    # main() 안에 타이머 실행
    global pub_timer
    pub_timer = rospy.Timer(rospy.Duration(1.0/max(1.0, fs)), lambda e: publish_all(e, one_based=one_based))
    ...
    # 퍼블리셔/구독자
    '''
    이렇게 하면 노드가 내려간 뒤에도 마지막 0값이 유지되어, 구독자가 이전의 비영(非0) 명령을 계속 보는 일이 없어집니다.
    /gps/GRADEUP_ON은 요구사항대로 latch=False 유지(새 구독자에게 과거 1이 남지 않게).
    '''
    pub_speed = rospy.Publisher(TOPIC_SPEED_CMD, Float32, queue_size=10, latch=True)
    pub_steer = rospy.Publisher(TOPIC_STEER_CMD, Float32, queue_size=10, latch=True)
    pub_rtk   = rospy.Publisher(TOPIC_RTK_STATUS, String,  queue_size=10, latch=True)
    pub_wpidx = rospy.Publisher(TOPIC_WP_INDEX,   Int32,   queue_size=10, latch=True)
    pub_grade = rospy.Publisher(TOPIC_WP_GRADEUP_ON, Int32, queue_size=10, latch=False)  # 그대로. 저장 안함
    ...
    try:
        while not rospy.is_shutdown(): # 셧다운 아닐때
            ...
            while not gps_queue.empty(): # gps 값 있을때 실행
                ...
                if heading_vec is not None:
                    # raw_heading (rad)
                    raw_heading_rad = math.atan2(heading_vec[1], heading_vec[0])

                    # target bearing (deg)
                    target_bearing_deg = math.degrees(math.atan2(ty - y, tx - x))

                    # 안정화 함수 사용 (deg 입력/출력)
                    chosen_heading_deg = select_stable_heading_simple(target_bearing_deg, math.degrees(raw_heading_rad))

                    # chosen_heading_deg를 벡터로 변환하여 last_heading_vec 갱신
                    chosen_rad = math.radians(chosen_heading_deg)
                    last_heading_vec = (math.cos(chosen_rad), math.sin(chosen_rad))

...
def grade_stop_can_start(wp_display):
    ...
    if grade_stop_started or grade_stop_done:
        return False, None
    for z in flag_zones:
    ...
    try:
        while not rospy.is_shutdown():
            ...
            while not gps_queue.empty():
                ...
                # ── 구배 정지 트리거(원샷) ──
                can_start, hit_zone = grade_stop_can_start(wp_display)
                if can_start and hit_zone is not None:
                    grade_stop_start(time.time(), hit_zone)

                # ── grade 토픽 값 계산(override 단계가 아닐 때만) ──
                if active_flag is not None and (active_flag.get('grade_topic') is not None):
                    try:

...
def expand_indices_within_radius(x, y, xs, ys, idx_center, radius):
    """
    idx_center(0-based)를 중심으로 좌우 확장하여 radius 이내인 인덱스 범위 반환.
    없으면 (None, None).
    """
    n = len(xs)
...
def compute_wp_index_windowed(x, y, xs, ys, start_idx,
    ...
    nearest_idx, nearest_dist = find_nearest_index_windowed(x, y, xs, ys, start_idx,
                                                            window_ahead=window_ahead, window_back=window_back)
...
def _compute_rtk_status_txt(msg: NavSatFix):
    try:
        s = msg.status.status
        # 장비에 맞게 조정 필요: 여기서는 s>=0 을 FIX로 취급
...
def gps_callback(msg: NavSatFix):
    ...
    # ── wp_index(1-based) 빠르게 계산 (룩어헤드 없음)
    xs = reduced_waypoints[0]; ys = reduced_waypoints[1]
    # 시작점은 last_nearest_idx 사용(초기 0). 필요시 nav_target_idx0로 바꿔도 됨.

...
# 셧다운 플래그/타이머 자리(타이머 미사용이지만 호환 위해 둠)
shutting_down = False
pub_timer = None
...
# ───────── 경사/구간 계산(부작용 없이 값만 반환) ─────────
reduced_waypoints = None  # main에서 로드

def _zone_index_1based(x, y):
    """경사 관련 원(4..11) 중 포함되는 것들의 '가장 큰 1-based 인덱스' 반환. 없으면 None."""
    inside = []
    for idx in range(GRADE_APPROACH_START, GRADE_EXIT_END + 1):  # 4..11
        j = idx - 1
        if j < reduced_waypoints.shape[1]:
            wx = reduced_waypoints[0][j]
            wy = reduced_waypoints[1][j]
            if distance_in_meters(x, y, wx, wy) <= TARGET_RADIUS:
                inside.append(idx)
    return max(inside) if inside else None

def _compute_wp_index_1based(x, y):
    zone = _zone_index_1based(x, y)
    return zone if zone is not None else 0
...
def _compute_grade_logic(x, y, steer_cmd):
    """
    구간에 따른 (steer_out, zone_1based, speed_cmd, grade_val)만 반환.
    6번 정지(최초 1회 4초) 논리 포함. 퍼블리시는 하지 않음.
    셧다운이면 (0.0, None, 0.0, 0) 반환.
    """
    ...
    if zone is not None:
        if GRADE_APPROACH_START <= zone <= GRADE_APPROACH_END:
            grade_val = 1
            speed_cmd = BASE_SPEED
        elif zone == GRADE_STOP_INDEX:
            grade_val = 1
            if not hold_done:
                if not hold_active:
                    hold_active = True
            ...
            grade_val = 1
            speed_cmd = BASE_SPEED
        elif GRADE_EXIT_START <= zone <= GRADE_EXIT_END:
            grade_val = 0
            speed_cmd = BASE_SPEED

    return steer_out, zone, speed_cmd, grade_val

# ───────── 콜백 (퍼블리시 중앙화: 여기서만 publish 호출) ─────────
def gps_callback(msg: NavSatFix):
    global nav_target_idx0, reduced_waypoints
    ...
    # 구간/경사 계산 (값만)
    steer_out, zone, speed_cmd, grade_val = _compute_grade_logic(x, y, smooth_inv)
    wp_index_1b = zone if zone is not None else 0

...
(경사 라치 + 정지 중 헤딩 스냅샷 고정 + 항상 퍼블리시 + 퍼블리시 중앙화
 + 안전 셧다운 + 윈도우 기반 wp_index 계산)

- 입력: (param) ~fix_topic  (sensor_msgs/NavSatFix, default=/gps1/fix)
- 출력(항상 퍼블리시, 루프당 각 1회):
  * /gps/steer_cmd      (std_msgs/Float32, deg)
  * /gps/wp_index       (std_msgs/Int32, 1-based. 반경 밖=0) ← 겹치면 "가장 큰 인덱스"
...
[경사 라치 로직 — 1-based index]
  - 4–5번 원 안:        speed=base_speed, steer=계산값, grade=1
  - 6번 원 최초 진입:    4초 hold( speed=0, steer=0, grade=1 ) → 이후 재정지 금지(라치)
...
[헤딩 처리]
  - 정지/저속/hold 중에는 GPS 샘플 2점으로 헤딩을 새로 계산하지 않고,
    "마지막으로 충분히 움직였을 때"의 헤딩 스냅샷을 고정 사용.
"""
...
TOPIC_RTK_STATUS    = '/gps/rtk_status'    # String ("FIX"/"NONE")  # 여기서는 FIX/NONE만 사용
TOPIC_WP_INDEX      = '/gps/wp_index'      # Int32 (1-based, 반경 밖=0)
TOPIC_WP_GRADEUP_ON = '/gps/GRADEUP_ON'    # Int32 (0/1)

# ───────── 상수/파라미터 ─────────
EARTH_RADIUS        = 6378137.0     # [m]
MIN_DISTANCE        = 2.8           # [m] 웨이포인트 간 최소 간격(리듀스 시)
...
# hold 종료 후 0을 조금 더 밀어주는 여유(통신/하위제어 잔상 제거)
POST_HOLD_ZERO_SEC   = 0.3

# 정지/저속 판정용 최소 이동거리(이상 움직였을 때만 헤딩 갱신)
HEADING_MOVE_MIN_M   = 0.04  # 4cm 정도
...
# ── 경사 정지 라칭 상태기 ─────────────────────────────
hold_active = False        # 현재 4초 정지 중
hold_done   = False        # 정지 1회 완료(재무장 금지)
hold_end_t  = 0.0          # hold 타이머 끝 시각
post_zero_until = 0.0      # hold 직후 0 강제 유지 종료 시각

# 헤딩 스냅샷(rad): 마지막으로 '충분히 움직였을 때'의 헤딩
...
def lpf_and_invert(current_angle_deg):
    """조향 각도 1차 LPF 후 부호 반전(차량 좌/우 기준 보정 시 사용)"""
...
def find_nearest_index_windowed(x, y, xs, ys, start_idx, window_ahead=80, window_back=15):
    ...
    sub_x = np.asarray(xs[i0:i1+1])
    sub_y = np.asarray(ys[i0:i1+1])
...
def compute_wp_index_windowed(x, y, xs, ys, start_idx,
    ...
    """
    윈도우 기반으로 빠르게 wp_index(1-based)를 계산.
    - 윈도우에서 최근접을 찾고, 반경 내 연속 구간을 좌우 확장해 '가장 큰 인덱스' 반환.
    - 윈도우 근처에 없고 최근접 거리가 너무 크면(이탈 추정) 드물게 전체 벡터화 폴백.
    """
...
def _compute_grade_logic_with_zone(zone_1b, steer_cmd):
    """
    zone_1b: 1-based 또는 None
    반환: (steer_out, zone_1b, speed_cmd, grade_val)
    """
    global hold_active, hold_done, hold_end_t, shutting_down, post_zero_until
    if shutting_down:
        return 0.0, None, 0.0, 0

    now = time.time()

    # ── [최우선] 정지 라치가 살아있으면 무조건 0 강제 ─────────────
    if hold_active and (now < hold_end_t):
        return 0.0, zone_1b, 0.0, 1

    # 정지 종료 직후, 하위제어 잔상 제거용 0 강제 구간
...
def gps_callback(msg: NavSatFix):
    ...
    # wp_index(1-based) 계산(룩어헤드 없음, 윈도우 가속)
    ...
    if len(current_x) >= 2:
        ...
        if (dist < HEADING_MOVE_MIN_M) or hold_active:
            if last_good_heading_rad is not None:
                ...
                # 스냅샷이 없으면 타깃 방향을 헤딩으로 가정(안전 fallback)
                head_vec_use = np.array([tx, ty]) - curr
        else:
            head_vec_use = dxy  # 평소엔 실제 이동 벡터 사용
    ...
    # 경사 제어 zone (4..11에서만 동작 — 라치는 위에서 선처리됨)
    zone_for_grade = wp_index_1b if (4 <= wp_index_1b <= 11) else None

    # 경사 로직 적용(라치/포스트-제로가 우선권 가짐)
    ...
    rospy.loginfo(
        "steer=%.2f°, speed=%.2f, grade=%d, wp_idx=%d (dt=%.3f, tau=%.3f, alpha=%.3f) "
        "| hold_active=%s hold_done=%s",
        steer_out, speed_cmd, grade_val, wp_index_1b, DT, TAU, ALPHA,
        str(hold_active), str(hold_done)
    )
...
def main():
    ...
    try:
        ...
        pkg_path = os.path.expanduser('~/catkin_ws/src/rtk_waypoint_tracker')
    csv_path = rospy.get_param('~csv_path', os.path.join(pkg_path, 'config', 'raw_track_latlon_18.csv'))
    _logs_dir = rospy.get_param('~logs_dir', os.path.join(pkg_path, 'logs'))
    _log_csv  = rospy.get_param('~log_csv',  os.path.join(_logs_dir, f"waypoint_log_{time.strftime('%Y%m%d_%H%M%S')}.csv"))

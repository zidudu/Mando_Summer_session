...
last_hold_zone_range = (0, 0)   # (start1, end1)  1-based

# 최종 도착 지점 정지 렌치 on off
...
#  - speed_code/speed_cap/step_per_loop: 속도 제어 덮어쓰기(옵션)
#  - stop_on_hit: 이 구간의 “웨이포인트 원에 들어온 순간” 정지 트리거 (언덕 등)
#  - stop_duration_sec: 구간별 정지 유지 시간(초), 미지정 시 STOP_FLAG_STAY_SEC 사용
# ──────────────────────────────────────────────────────────────────
FLAG_DEFS = [
...
zone_armed = True            # 같은 원에서 중복 트리거 방지(원 밖→안 전이 시 재무장)

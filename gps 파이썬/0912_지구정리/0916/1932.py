...
def _on_shutdown():
    ...
    # pub_wpidx = 0
    # pub_grade = 0  
    """노드 종료 시 모든 제어 토픽을 0으로 퍼블리시하고,
       구배 토픽(정식/레거시)도 래치 0으로 못박는다."""
    global speed_cmd_current_code, last_pub_speed_code
    rospy.loginfo("[tracker] shutdown: ramping to 0 and latching zeros")

    try:
        # 1) 비래치 퍼블리셔로 잠깐 0 연속 송신(네트워크 지연 대비)
        speed_cmd_current_code = 0
        last_pub_speed_code = 0.0
        pub_speed.publish(Float32(0.0))
        pub_steer.publish(Float32(0.0))
        pub_wpidx.publish(Int32(0))
        pub_grade.publish(Int32(0))  # 래치 퍼블도 덮어쓰기
        #if pub_rtk:   pub_rtk.publish(String("NONE"))

        # # 2) 래치 퍼블리셔들로 마지막 0을 남김(정식 + 레거시)
        # latch_speed   = rospy.Publisher(TOPIC_SPEED_CMD,        Float32, queue_size=1, latch=False)
        # latch_steer   = rospy.Publisher(TOPIC_STEER_CMD,        Float32, queue_size=1, latch=False)
        # latch_wpidx   = rospy.Publisher(TOPIC_WP_INDEX,         Int32,   queue_size=1, latch=False)
        # latch_grade   = rospy.Publisher(TOPIC_WP_GRADEUP_ON,      Int32,   queue_size=1, latch=False)
        ...
        while not rospy.is_shutdown():
            ...
            while not gps_queue.empty():
                ...
                if stop_zone_now:
                    ...
                    # 현재 어떤 플래그도 활성화되어 있지 않으면(= 구간 완전 탈출) 재무장
                    if active_flag is None:
                        zone_armed = False
                        last_hold_zone_name = None
                ...
                #desired = max(0, min(cap, desired)) # 최대 제한 안넘어가게 함
                #speed_desired_code = desired
                speed_cmd_current_code = desired  # ← 즉시 점프

...
MIN_DISTANCE = 2.0       # wp 마다의 간격
TARGET_RADIUS = 2.0      # wp 원 반경
ANGLE_LIMIT_DEG = 27.0   # 조향각 제한

# Grade 구간 정의(모두 1-based, '타겟 인덱스' 기준)
GRADE_APPROACH_START = 100
GRADE_APPROACH_END   = 101
GRADE_STOP_INDEX     = 102
GRADE_EXIT_START     = 103
GRADE_EXIT_END       = 103
# 완전 정지 인덱스
STOP = 104
...
# 셧다운 플래그 (True면 콜백에서 로깅/퍼블리시 건너뜀)
_shutting_down = False

# 주석 이유 : dt는 0.05로 고정해서 필요없음
...
# ───────── 구간별 속도 하드코딩 (1-based 인덱스) ─────────
# 형식: {'start': <1-based start idx>, 'end': <1-based end idx>, 'speed': <m/s>}
ZONE_DEFS = [
    #{'start': 1, 'end': 4, 'speed': 4.0},  # WP 1~4 구간: 3.0 m/s
    #{'start': 5, 'end': 6, 'speed': 0.0},  # WP 5~6 구간: 정지 (예: 구배)
    #{'start': 7, 'end': 12, 'speed': 4.0}, # WP 7~12 구간: 4.0 m/s
    # 필요하면 더 추가하세요...
]
...
# 구간별 속도 정하기 함수
def get_zone_speed(target_1b):
    """target_1b(1-based)에 해당하는 구간 속도 반환. 없으면 None."""
    # 타겟이 None, 즉 없으면 None 반환
    if target_1b is None:
        return None
    # 존 안에 있는 값 for 문 돌림
    for z in ZONE_DEFS:
        # 타겟 위치가 구간에 있다면 그 속도로 반영
        if z['start'] <= int(target_1b) <= z['end']:
            # 속도가 9 이상이면 (10이 최대) 램핑
            return float(z['speed'])
    return None
...
def _grade_logic_by_target(target_1b, steer_cmd):
    ...
    #speed_cmd = BASE_SPEED  # 고정 속도
    # 기본 speed: 구간별 우선, 없으면 BASE_SPEED 사용
    zone_speed = get_zone_speed(target_1b) if (target_1b is not None) else None  # 구간 속도
    speed_cmd = zone_speed if (zone_speed is not None) else BASE_SPEED

    # 구배 토픽
    grade_val = 0  # 기본 0

    # 조향각
    steer_out = steer_cmd  # 계산된 매개변수 값 사용

    # 접근/탈출 동안 grade=1 유지
    if target_1b is not None:
        if GRADE_APPROACH_START <= target_1b < GRADE_EXIT_START:
            grade_val = 1
            # 주석 단 이유 : 위에서 grade가 0인데 조건식에서 범위 안에 있으면 1로 덮어씌움.
            # 조건이 아닐땐 무조건 0이 나오는데 이 조건식은 의미가 없음.쓸데없는 연산만 늘어남.
            # grade가 1일땐 구배 구간에서만 쓰이기 때문임.
...
def log_csv_row(lat, lon, heading_deg, steer_pub_deg, speed_pub, grade_pub, tgt_idx_pub, rtk_txt):
    """요청 필드만 정확히 한 줄 기록. 셧다운 중이거나 파일이 없으면 아무 동작 안 함."""
    global _log_fh, _log_writer, _log_count, _shutting_down
    if _shutting_down:
        return
    if _log_writer is None or _log_fh is None:
        return
    try:
        _log_writer.writerow([
        ...
        # 파일이 닫혀 있었을 수 있음(경합). 안전하게 무시.
        return
    except Exception as e:
        rospy.logwarn("[log] CSV write failed: %s", e)

# ───────── NavSatFix 콜백(유일한 계산 루프) ─────────
def gps_callback(msg: NavSatFix):
    # global _last_fix_walltime
    global ALPHA
    global last_good_heading_rad, nav_target_idx0
    global hold_active, hold_done, hold_end_t, restart_steer_zero_until

    # 노드 셧다운 진행 중이면 콜백 무시
    if _shutting_down or rospy.is_shutdown():
        return
    ...
    if len(current_x) >= 2:
        ...
        # 이동량 있을때 사용(기본 주행때 사용한다 보면 됨)
        if dist >= HEADING_MOVE_MIN_M:
            last_good_heading_rad = math.atan2(dxy[1], dxy[0])

        # 정지 상태이거나 이동량이 적을때 스냅샷이 있으면 그 스냅샷 헤딩을 단위벡터로 만들어 사용
        # 스냅샷 헤딩은 기본 이동때 헤딩을 얻어냄.
        if (dist < HEADING_MOVE_MIN_M) or hold_active:
            if last_good_heading_rad is not None:
                head_vec_use = np.array([math.cos(last_good_heading_rad), math.sin(last_good_heading_rad)])
            else:
    ...
    # 현재 위치 (x,y)에서 타겟(목표 웨이포인트) (tx,ty) 까지의 벡터(방향)를 만듭니다.
    # => 차에서 목표로 가는 직선 방향
    # 타겟 벡터
    ...
    rospy.loginfo(
        ...
        dt, steer_out, speed_cmd, grade_val, target_1b, rtk_txt, hold_active, hold_done,
...
def _on_shutdown():
    global _log_fh, _log_writer, _shutting_down
    # 1) 셧다운 플래그 먼저 올려 콜백에서 로깅/퍼블리시를 건너뛰도록 함
    _shutting_down = True
    try:
        for _ in range(2):
            _publish_zero_once()
    ...
    try:
        ...
        rospy.logwarn("[log] CSV close failed: %s", e)
...
def main():
    ...
    enable_plot = bool(rospy.get_param('~enable_plot', ENABLE_PLOT))
    ...
    rospy.loginfo("Mode: Callback-only (time-based latches) | tau=%.3fs | base_speed=%.2f",
                  TAU, BASE_SPEED)

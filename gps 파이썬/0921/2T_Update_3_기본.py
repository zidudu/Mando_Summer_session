...
# -*- coding: utf-8 -*-
"""
ROS1 · GPS waypoint 추종 (콜백 기반, 시간기반 라치 복구)
- 콜백에서만 처리합니다.
...
def _compute_rtk_status_txt(msg: NavSatFix):
    try:
        s = msg.status.status
        if s is not None and s >= 2: # 토픽 2면 FIX
            return "FIX"
        elif s is not None and s == 1: # 토픽 1이면 FLOAT
            return "FLOAT"
        else:
            return "NONE" 
    except Exception:
...
def _grade_logic_by_target(target_1b, steer_cmd):
    ...
    # 접근/탈출 구간에 따른 grade 토픽
    if target_1b is not None:
        if GRADE_APPROACH_START <= target_1b < GRADE_EXIT_START: # 구배 시작구간부터 구배 끝나는 구간
            grade_val = 1
        elif GRADE_EXIT_START <= target_1b <= GRADE_EXIT_END:
            grade_val = 0
        # 구간 지정 없을땐 grade_val = 0
        else:
            grade_val = 0
    # 완전정지 구간
    if target_1b is not None:
        if target_1b == STOP: # 타겟이 정지구간일때
            # 속도, 조향 아예 0
            steer_cmd = 0
            speed_cmd = 0
            grade_val = 0 
...
def gps_callback(msg: NavSatFix):
    ...
    # 퍼블리시
    #if pub_rtk:   pub_rtk.publish(String(_compute_rtk_status_txt(msg)))

...
STEP_PER_LOOP_DEFAULT = 1     # 매 루프당 정수 스텝(±1이 기본)
...
#  - spacing_scale: 웨이포인트 간격 스케일(예: 0.5면 절반으로 촘촘). 사용 안하면 None
# ──────────────────────────────────────────────────────────────────
FLAG_DEFS = [
    # 예시) 구배(GRADE_UP): 반경/룩어헤드 축소 + 간격 절반(2.5m → 1.25m)
    {'name': 'GRADE_UP', 'start': 10, 'end': 20,
     'radius_scale': 0.5, 'lookahead_scale': 0.8,
     'spacing_scale': 0.5,             # ← 촘촘
     'speed_code': 2, 'speed_cap': 3, 'step_per_loop': 1},

    # S자 구간: 룩어헤드 소폭 축소, 간격은 기본(None)
    {'name': 'S_CURVE', 'start': 20, 'end': 28,
     'radius_scale': 0.9, 'lookahead_scale': 0.9,
     'spacing_scale': None,            # ← 간격 그대로
     'speed_code': 2, 'speed_cap': 3, 'step_per_loop': 1},

    # 우회전 직각 구간
    {'name': 'RIGHT_ANGLE', 'start': 35, 'end': 36,
    'radius_scale': 0.8, 'lookahead_scale': 0.8,
    'spacing_scale': None,
    'speed_code': 1, 'speed_cap': 2, 'step_per_loop': 1},
    ...
    # 직선 가속
    {'name': 'STRAIGHT_FAST', 'start': 70, 'end': 80,
     'radius_scale': 1.0, 'lookahead_scale': 1.0,
     'spacing_scale': None,
     'speed_code': 4, 'speed_cap': 6, 'step_per_loop': 1},
]
...
def build_flag_zones(flag_defs):
    ...
    for fd in flag_defs:
        ...
        spacing_scale_val = fd.get('spacing_scale', None)
        if spacing_scale_val is None:
            sscale = None
        else:
            try:
                sscale = float(spacing_scale_val)
            except Exception:
                sscale = None
        zones.append({
            'name': fd['name'],
...
def main():
    ...
    base_speed_code_param = int(rospy.get_param('~speed_code', 2))
    ...
    try:
        while not rospy.is_shutdown():
            ...
            while not gps_queue.empty():
                ...
                # 플래그 상태 갱신 → 반경/룩어헤드/속도/spacing 스케일 정보
                ...
                # ── spacing_scale 적용: 타깃 인덱스 계산에만 사용 ──
                if z_active is not None and z_active.get('spacing_scale') is not None:
                    spacing_eff = max(1e-3, WAYPOINT_SPACING * float(z_active['spacing_scale']))
                else:
                    spacing_eff = WAYPOINT_SPACING

                tgt_idx = target_index_from_lookahead(nearest_idx, Ld, spacing_eff, len(spaced_x))

...
'''
0911_ 수정한거 (시각화만 plt.ion 방식으로 교체)
1) 교차로 구간 순차 인덱스 모드 (유지)
2) Matplotlib 실시간 갱신: FuncAnimation → plt.ion + draw_idle/plt.pause
3) 퍼블리시 토픽 교체 + 구배정지 상태 토픽 추가 (유지)
...
# from matplotlib import animation  # ← 더 이상 사용하지 않음
...
# (이전 코드 호환용) 애니메이션이 읽을 타깃 인덱스 공유 변수 — 이제 직접 갱신
tgt_idx_for_viz = 0
    ...
    try:
        while not rospy.is_shutdown():
            ...
            while not gps_queue.empty():
                ...
                # 애니메이션 공유 변수(호환): 지금은 직접 그리지만 값은 유지

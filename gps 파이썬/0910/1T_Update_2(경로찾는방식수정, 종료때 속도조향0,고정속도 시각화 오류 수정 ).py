...
- 출력:
  ...
  * /vehicle/speed_cmd       (std_msgs/Float32)  → 고정 속도 값 퍼블리시(요청 추가)
  * /rtk/status              (std_msgs/String)   → "FIXED"/"FLOAT"/"NONE"(요청 추가)
- 파라미터:
  * ~csv_path     (str)   : 웨이포인트 CSV 경로 (기본: $(find rtk_waypoint_tracker)/config/raw_track_latlon_6.csv)
  ...
  * ~log_csv      (str)   : 로그 CSV 경로 (기본: $(find rtk_waypoint_tracker)/logs/waypoint_log_YYYYmmdd_HHMMSS.csv)
"""
...
# ---- 고정 속도 (여기를 바꾸면 범례/INFO/퍼블리시 전부 동일하게 변경됩니다) ----
FIXED_SPEED = 2.0  # [m/s] — 항상 이 고정값을 퍼블리시하고 범례/로그에도 이 값만 사용
...
def lpf_and_invert(current_angle_deg):
    """
    y[k] = (1-ALPHA)*y[k-1] + ALPHA*x[k],  (ALPHA = DT/(TAU+DT))
    이후 '부호 반전 유지' 적용.
    """
...
def gps_callback(msg: NavSatFix):
    ...
    if len(current_x) >= 2:
        ...
        smooth_inv = lpf_and_invert(angle_deg)  # LPF + 부호 반전

        # 퍼블리시(원본 + 추가: 속도, RTK) — 속도는 항상 FIXED_SPEED 사용
        steering_pub.publish(Float32(smooth_inv))
        if speed_pub:
            speed_pub.publish(Float32(FIXED_SPEED))  # 고정값만 퍼블리시
        if rtk_pub:
...
def update_plot(_):
    ...
    if current_x and current_y:
        ...
        # 동적 텍스트만을 위한 더미 라인 핸들들 — 속도는 항상 FIXED_SPEED 표시

...
0911_ 수정한거
1) 교차로 구간 순차 인덱스 모드
2) Matplotlib 실시간 갱신을 FuncAnimation으로 변경(터미널 점유 방지)
3) 퍼블리시 토픽 교체 + 구배정지 상태 토픽 추가
...
구배정지 상태:
TOPIC_WP_GRADEUP_ON = '/gps/GRADEUP_ON'   # Int32, 평소 0, GRADE_UP 홀드 중 1
'''
...
TOPIC_RTK_STATUS  = '/gps/rtk_status'
TOPIC_WP_INDEX    = '/gps/wp_index'
TOPIC_WP_GRADEUP_ON = '/gps/GRADEUP_ON'   # ★ 추가: 구배정지 상태(0/1)
...
FLAG_DEFS = [
    {'name': 'GRADE_UP', 'start': 14, 'end': 14,
     ...
     'stop_on_hit': True, 'stop_duration_sec': None, 'sequential': False},
    ...
    {'name': 'STRAIGHT_FAST', 'start': 1, 'end': 10,
     ...
     'speed_code': 8, 'speed_cap': 10, 'step_per_loop': 2, 'sequential': False},
...
pub_rtk   = None
pub_wpidx = None
pub_grade = None   # ★ GRADE_UP 상태 퍼블리셔
...
def publish_all(event, one_based=True):
    ...
    if hold_active and now >= hold_until:
        # hold_active 자체 해제는 메인 루프에서 이루어짐(상태 연속성 유지)
        pass
    ...
    # ★ 구배정지 상태 퍼블리시: hold가 GRADE_UP 사유일 때만 1, 아니면 0
    if pub_grade:
        grade_flag = 1 if (hold_active and (hold_reason == "GRADE_UP")) else 0
        pub_grade.publish(Int32(int(grade_flag)))
...
def _on_shutdown():
    ...
    try:
        ...
        while speed_cmd_current_code > 0:
            speed_cmd_current_code = max(0, speed_cmd_current_code - 2)
            if pub_speed: pub_speed.publish(Float32(float(speed_cmd_current_code)))
...
def main():
    ...
    enable_plot  = bool(rospy.get_param('~enable_plot', True))
    ...
    pub_grade = rospy.Publisher(TOPIC_WP_GRADEUP_ON, Int32, queue_size=10)  # ★ 추가
    ...
    # ── 애니메이션 콜백: 플롯은 여기서만 갱신 ──────────────────────
    last_heading_vec = None  # 이 변수는 아래 컨트롤 루프에서 갱신됨
    def _draw_frame(_):
        nonlocal last_heading_vec
        # 현재 위치
        if len(pos_buf) > 0:
            _, cx, cy = pos_buf[-1]
        else:
        ...
        # 타깃
        ti = max(0, min(int(tgt_idx_for_viz), len(spaced_x)-1))
        tx, ty = spaced_x[ti], spaced_y[ti]

        # 라인/마커
        live_line.set_data([p[1] for p in pos_buf], [p[2] for p in pos_buf])
        current_pt.set_data([cx], [cy])
        ...
        status = [
            ...
            f"Meas v={(float(np.median(speed_buf)) if speed_buf else 0.0):.2f} m/s",
        ...
        hud_text.set_text(
            f"TGT={ti+1} | pub_v={last_pub_speed_code:.1f} "
        ...
        # 순차모드 배지
        seq_badge.set_visible(bool(seq_active))

        return (live_line, current_pt, target_line, target_pt,)

    if enable_plot:
        ani = animation.FuncAnimation(fig, _draw_frame, interval=150)
    ...
    try:
        while not rospy.is_shutdown():
            ...
            while not gps_queue.empty():
                ...
                # 순차모드 동작: 도달 시 +1, 타깃은 seq_idx로 고정
                    ...
                    try:
                        ...
                        with open(log_csv_path, 'a', newline='') as f:
                            ...
                            w.writerow([time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
                                        ...
                                        wp_display, int(tgt_idx+1), f"{tx:.3f}", f"{ty:.3f}",
                                        ...
                                        1 if (hold_active and hold_reason=="GRADE_UP") else 0])
                        _last_log_wall = noww
                    except Exception as e:
                ...
                rospy.loginfo_throttle(
                    0.5,
                    f"TGT={int(tgt_idx+1)} | WP(in-radius)={wp_display} / tgt={int(tgt_idx+1)}/{len(spaced_x)} | "
                    ...
                    + (f" | GRADEUP_ON=1" if (hold_active and hold_reason=='GRADE_UP') else "")
                )

            # 애니메이션은 GUI 타이머가 알아서 그림 갱신 → 여기서는 ROS 루프만 돌면 됨
            rate.sleep()

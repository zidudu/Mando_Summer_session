...
0911_ 수정(요청 반영)
- 시각화: plt.ion 방식 유지
- 플래그에서 /gps/GRADE_UP_ON 값을 직접 제어하도록 변경 (grade_topic)
- 홀드 5초 후 자동 0 전송 제거(토픽 값은 플래그에 의해 결정)
- 토픽명: TOPIC_GRADE_UP_ON = '/gps/GRADE_UP_ON'
'''
...
LPF_FC_HZ          = 0.8
SPEED_BUF_LEN      = 10
MAX_JITTER_SPEED   = 8.0 # 4에서 8로 바꿈
MIN_MOVE_FOR_HEADING = 0.05

FS_DEFAULT         = 20.0
GPS_TIMEOUT_SEC    = 1.0

STOP_FLAG_STAY_SEC = 3.0 # 구배 정지 시간
...
TOPIC_GRADE_UP_ON   = '/gps/GRADEUP_ON'   # ★ 플래그가 직접 0/1 제어
...
# ──────────────────────────────────────────────────────────────────
# 플래그(구간) 정의
#  - grade_topic: 1/0/None
#     · 1/0 → 해당 구간 활성 시 /gps/GRADE_UP_ON 토픽에 그 값을 퍼블리시
#     · None → 이 구간은 GRADE_UP 토픽에 영향 주지 않음(기본 0 유지)
# ──────────────────────────────────────────────────────────────────
FLAG_DEFS = [
...
pub_rtk   = None
pub_wpidx = None
pub_grade = None   # ★ /gps/GRADE_UP_ON 퍼블리셔
...
# 플래그가 직접 제어하는 GRADE_UP 토픽 값(기본 0)
grade_topic_value = 0
...
def publish_all(event, one_based=True):
    ...
    # ★ 플래그가 직접 제어한 GRADE_UP 토픽 값 퍼블리시
    if pub_grade:
...
def main():
    ...
    global tgt_idx_for_viz, grade_topic_value
    ...
    # 정지 유지시간 (홀드는 속도용 로직으로만 사용, GRADE_UP 토픽은 영향 X)
    ...
    pub_grade = rospy.Publisher(TOPIC_GRADE_UP_ON, Int32, queue_size=10)  # ★ 변경된 토픽명
    ...
    try:
        while not rospy.is_shutdown():
            ...
            while not gps_queue.empty():
                ...
                # ★ 플래그가 GRADE_UP 토픽 값을 직접 지정
                #   - active_flag가 있고 해당 zone에 grade_topic가 설정되어 있으면 그 값(0/1)
                #   - 아니면 기본값 0
                ...
                # (선택) stop_on_hit 로직: 속도 정지는 유지하되 GRADE_UP 토픽은 영향 없음
                stop_zone_now = None
                if wp_display > 0:
                    for z in flag_zones:
                ...
                if enable_plot:
                    ...
                    hud_text.set_text(
                        f"TGT={tgt_idx+1} | pub_v={float(speed_cmd_current_code if not hold_active else 0):.1f} "
                    ...
                    try:
                        ...
                        with open(log_csv_path, 'a', newline='') as f:
                            ...
                            w.writerow([time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
                                        ...
                                        int(grade_topic_value)])   # ★ 토픽 값 그대로 로깅
                        _last_log_wall = noww
                    except Exception as e:
                ...
                rospy.loginfo_throttle(
                    0.5,
                    f"TGT={int(tgt_idx+1)} | WP(in-radius)={wp_display} | "

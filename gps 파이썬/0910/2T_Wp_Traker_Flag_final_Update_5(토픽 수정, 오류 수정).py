...
=> 이 코드에서 오류가 자주 나면 밑에 있는걸 쓰기

#진입 시 시작 인덱스 클램프
#드물게 nearest_idx가 구간 끝을 살짝 넘은 상태에서 들어오는 엣지케이스 방어:
# 진입 처리 때
seq_idx = max(z_active['start0'], min(nearest_idx, z_active['end0']))

2.
                # 0911 ★ 순차모드 활성 시: 도달하면 +1, 타깃은 항상 seq_idx
                if seq_active and (seq_zone is not None):
                    # 현재 원(반경) 안에 들어왔으면 다음 인덱스로 전진 (구간 끝 넘지 않게)
                    if wp_index_active == seq_idx:
                        seq_idx = min(seq_idx + 1, seq_zone['end0'])
                     
                    # -> 반경 검출 실패 시 불필요한 증가 방지. 위에꺼 불필요 인덱스 증가하면 이걸로
                    #if (wp_index_active >= 0) and (wp_index_active == seq_idx):
                    #   seq_idx = min(seq_idx + 1, seq_zone['end0'])

3.

# 퍼블리시 토픽
# TOPIC_SPEED_CMD    = '/vehicle/speed_cmd'     # Float32 (정수코드 값을 실수로 송신)
...
TOPIC_SPEED_CMD = '/gps/speed_cmd'    # Float32 (정수코드 값을 실수로 송신)
...
# ──────────────────────────────────────────────────────────────────
# CSV 원본 경로를 따라 “보간해서 만든” 주행용 웨이포인트 간격입니다.
WAYPOINT_SPACING   = 2.5         # m
# 차량이 “해당 웨이포인트에 도달했다”고 판정하는 반경입니다.
...
# 룩어헤드
LOOKAHEAD_MIN      = 3.2         # m  (룩어헤드 최소값)
LOOKAHEAD_MAX      = 4.0         # m  (룩어헤드 최대값)
LOOKAHEAD_K        = 0.2         # m per (m/s) — 속도에 비례해 룩어헤드가 늘어나는 계수

LPF_FC_HZ          = 0.8         # Hz (조향 각 LPF의 기준 컷오프 주파수)
SPEED_BUF_LEN      = 10          # 속도 추정에 사용되는 이동평균 버퍼 길이 (샘플 수)
MAX_JITTER_SPEED   = 4.0         # m/s — 순간 속도 추정에서 스파이크(이상치)로 간주할 최대값
MIN_MOVE_FOR_HEADING = 0.05      # m — 최근 위치들로부터 헤딩(방향)을 유효하게 추정하기 위한 최소 이동거리

FS_DEFAULT         = 20.0        # 퍼블리시/루프 Hz
GPS_TIMEOUT_SEC    = 1.0         # 최근 fix 없을 때 안전정지

# ── 언덕 정지 시간(초) ──────────────────────────────────────────────
# 구간에 stop_duration_sec를 따로 지정하지 않으면 이 값을 기본으로 사용합니다.
STOP_FLAG_STAY_SEC = 3.0

# ── 속도 명령 상수 ────────────────────────────────────────────────
# None이면 rosparam(~speed_code)을 기본으로 사용
SPEED_FORCE_CODE = None
SPEED_CAP_CODE_DEFAULT = 10
STEP_PER_LOOP_DEFAULT  = 2

# 기본 속도. 플래그 구간이 아니거나 speed_code가 None이면 이 값을 씀
BASE_SPEED       = 5   # 기본 speed_code (정수 코드)
...
#  - speed_code: 구간 내 사용할 속도 코드(0~N). None이면 기본(speed param) 사용
#  - speed_cap:  구간 내 상한(선택)
#  - step_per_loop: 매 루프당 코드 변화 스텝(정수)
#  - stop_on_hit: 해당 웨이포인트 원에 들어온 순간 정지(hold) 트리거
#  - sequential: 교차로 등에서 '순차 인덱스 모드'를 켤지 여부
# ──────────────────────────────────────────────────────────────────
FLAG_DEFS = [
    ...
    {'name': 'INTERSECTION_1', 'start': 30, 'end': 40,
     'radius_scale': 1.0, 'lookahead_scale': 1.0,
     'speed_code': None, 'speed_cap': None, 'step_per_loop': 2,
     'sequential': True},   # ★ 추가 키, None이면 BASE_SPEED 값 사용, sequential True는 이때 순차인덱스 on, False면 순차인덱스 off
]
...
# ──────────────────────────────────────────────────────────────────
# 전역 상태 (런타임에서 변경되는 변수들)
# ──────────────────────────────────────────────────────────────────
gps_queue = Queue()                      # 수신된 NavSatFix를 임시 저장하는 큐 (producer/consumer 패턴)
latest_filtered_angle = 0.0              # Float: LPF를 통과한(및 SIGN 적용된) 최종 조향 값(퍼블리시용)
pos_buf   = deque(maxlen=SPEED_BUF_LEN*2) # 최근 위치(t,x,y)들을 저장하여 헤딩/속도 계산에 사용
speed_buf = deque(maxlen=SPEED_BUF_LEN)  # 최근 순간속도(instant v)들을 저장하여 중앙값/평가에 사용

pub_speed = None    # rospy.Publisher 객체 (TOPIC_SPEED_CMD)
pub_steer = None    # rospy.Publisher 객체 (TOPIC_STEER_CMD)
pub_rtk   = None    # rospy.Publisher 객체 (TOPIC_RTK_STATUS)
pub_wpidx = None    # rospy.Publisher 객체 (TOPIC_WP_INDEX)

rtk_status_txt = "NONE"  # String: RTK 상태 텍스트 (FIX/FLOAT/NONE)
last_fix_time  = 0.0     # wall-time 마지막 GPS fix 수신 시각 (time.time())
...
# 플래그(구간) 런타임 상태
flag_zones = []              # build_flag_zones()로 변환된 내부 리스트(0-based)
active_flag = None           # 현재 활성 플래그 dict (None이면 구간 비활성)
just_entered = False         # 최근 루프에서 플래그 진입 여부
just_exited = False          # 최근 루프에서 플래그 이탈 여부

# 로그
log_csv_path = None
_last_log_wall = 0.0         # 마지막 로그 기록 wall-time

# ── 언덕(hold) 정지 관련 상태(홀드) ────────────────────────────────
last_hold_zone_name = None         # 마지막으로 홀드가 걸렸던 zone 이름
last_hold_zone_range = (0, 0)      # (start1, end1) 1-based 범위 표기(디버깅/로그용)

# 최종 도착 지점 정지 플래그(한번 정지 후 latch)
final_stop_latched = False

hold_active = False       # 현재 hold 중(정지 중)이면 True
hold_until  = 0.0         # hold 종료 wall-time (time.time() 기준)
hold_reason = ""          # hold 발생 사유(예: "GRADE_UP")
last_hold_wp_idx = 0      # 마지막으로 hold를 유발한 1-based WP 인덱스
zone_armed = True         # 동일 원에서 중복 트리거 방지(원 밖→안 될 때만 재무장)

# ── 0911 교차로 순차 인덱스 상태 ────────────────────────────────────────
seq_active = False        # 순차 모드 활성화 여부 (intersection follow mode)
seq_idx    = -1           # 현재 따라갈 0-based 인덱스 (seq_active일 때 사용)
seq_zone   = None         # 현재 순차 플래그(zone dict)
_prev_seq_active = False  # 이전 루프의 seq_active (UI/범례 업데이트용)
...
def build_flag_zones(flag_defs):
    """1-based → 0-based로 변환하여 내부 구조로 보관 + stop 옵션 병합"""
    zones = []
    for fd in flag_defs:
        ...
        zones.append({
            ...
            'sequential': bool(fd.get('sequential', False)),   # ★ 추가
    ...
    if active_flag is None:
        for z in flag_zones:
            if in_radius_idx(x, y, z['start0'], xs, ys, base_radius):
                ...
                flag_enter(z)
                break
    # 2) 종료 체크(최근접 인덱스가 구간 end를 넘기면 종료)
    else:
        z = active_flag
...
def main():
    ...
    # ★ 추가
    global seq_active, seq_zone, seq_idx
    ...
    try:
        while not rospy.is_shutdown():
            ...
            while not gps_queue.empty():
                ...
                if entered and (z_active is not None) and z_active.get('sequential', False):
                    seq_active = True
                    seq_zone   = z_active
                    # 진입 시점 기준으로 뒤로는 안가도록 start0와 최근접 중 큰 쪽으로 시작
                    # 반경 내 인덱스가 있으면 그걸, 없으면 nearest_idx 사용 => 근데 애매. 필요하면 넣기

                    # start_hint = wp_index_active if wp_index_active >= 0 else nearest_idx
                    seq_idx    = max(z_active['start0'], nearest_idx)

                    # 애매하면 이거쓰기 :순차 모드 진입 시 시작 인덱스 clamp,(혹시 nearest가 end0을 넘는 엣지 케이스 방지)
                    # seq_idx = max(z_active['start0'], min(nearest_idx, z_active['end0'])) 
                    rospy.loginfo(f"[seq] ENTER sequential mode {z_active['name']} {z_active['disp_range']} (start idx={seq_idx+1})")
                ...
                # 퍼블리시/표시용 인덱스(반경 내면 인덱스, 아니면 -1)
                wp_index_active = nearest_in_radius_index(x, y, spaced_x, spaced_y, eff_radius)
                wp_display = (wp_index_active + 1) if (wp_index_active >= 0) else 0  # 1-based
                ...
                if seq_active and (seq_zone is not None):
                    ...
                    # 타깃 인덱스는 룩어헤드 무시하고 '현재 seq_idx'로 고정
                    tgt_idx = seq_idx
                    tx, ty = spaced_x[tgt_idx], spaced_y[tgt_idx]
                    target_line.set_data([x, tx], [y, ty])
                    target_pt.set_data([tx], [ty])
                    ...
                    # (중요) 다음 프레임의 최근접 탐색 앵커도 순차 인덱스로 고정
                ...
                # ── 언덕 등: stop_on_hit 트리거 ───────────────────────────
                # 현재 위치가 '어떤 stop_on_hit 구간의 1-based 범위'에 있는 WP 원에 들어왔다면,
                # 원 밖→안으로 전이될 때 한 번만 홀드 시작
                stop_zone_now = None
                if wp_display > 0:
                    for z in flag_zones:
                        if z.get('stop_on_hit', False):
                            start1, end1 = (z['start0']+1), (z['end0']+1)  # 1-based
                ...
                if stop_zone_now:
                    if zone_armed and not hold_active:
                        ...
                        zone_armed = False  # 같은 원에서 재트리거 방지
                        # 홀드 시작 시 목표 속도도 0으로 설정(재가속은 루프에서 자연 램핑)
                ...
                # # ── 재무장 조건 변경(원 밖이면 재무장 → “구간 밖이면 재무장”) ──
                # 기존: else: zone_armed = True        
                else:
                    # 원 밖이라도 같은 구간 범위 안이면 재무장하지 않음
                    if last_hold_zone_name is None:
                        zone_armed = True
                    else:
                        s1, e1 = last_hold_zone_range
                        # 현재 표시 인덱스가 구간 범위를 벗어나면 재무장
                ...
                # 홀드 중엔 목표 속도 0 유지(퍼블리시도 0,0으로 오버라이드됨)
                if hold_active:
                    desired = 0
                    cap = 0
                ...
                # 속도 명령 코드(speed_cmd_current_code)를 목표값(speed_desired_code) 쪽으로 단계적으로(ramp) 보간
                if speed_cmd_current_code < speed_desired_code:
                    # 현재 코드가 목표보다 작으면 → step_this_loop 만큼 증가 (급출발 방지)
                ...
                # (0911 교체 전)
                # hud_text.set_text(
                #     f"TGT={tgt_display} | pub_v={float(speed_cmd_current_code if not hold_active else 0):.1f} "
                #     f"| meas_v={speed_mps:.2f} m/s | steer={latest_filtered_angle:+.1f}° "
                #     f"| WP={wp_display}" + (f" | {hold_txt}" if hold_txt else "")
                # )

                # (0911 교체 후)  ⬅︎ 실제 퍼블리시된 코드값만 표시
                hud_text.set_text(
        ...
        _on_shutdown() # 종료 시 속도/조향 0 퍼블리시

...
# -*- coding: utf-8 -*-
"""
waypoints_live_gngga_viewer_seq_tx.py  (로그 저장 + arrive_r 벗어날 때만 복귀)
• INIT→TRACK 상태머신 + Pure-Pursuit + 이중 반경 히스테리시스
• 매 프레임 로그 수집, 종료 시 CSV 저장
• arrive_r(도달 반경) 벗어나면 주변에서 가장 큰 인덱스 웨이포인트로 재진입
"""
...
CONFIG = {
    ...
    'k_ld':         0.3,
    'k_v':          0.5,
    'arrive_r':     0.5,     # 내부 반경 (도달 반경)
    'exit_r':       0.8,
    'tx_period':    0.2,
    'tx_on':        True,
    'const_speed':  1,
    'route_maxlen': 10000,
    'log_csv':      'gps_wp_log.csv',
    'v_th':         0.1,
}
...
def find_rejoin_wp(cx, cy, wx, wy, yaw, delta):
    # 자신의 주변 arrive_r 반경 내에서 가장 큰 인덱스 선택
    search_radius = CONFIG['arrive_r'] * 2.0
    dists = np.hypot(wx-cx, wy-cy)
    cands = np.where(dists <= search_radius)[0]
    return int(cands.max()) if cands.size>0 else int(dists.argmin())
...
def init_plot(df, wx, wy):
    ...
    for i,(x,y) in enumerate(zip(wx,wy),1):
        ...
        ax.add_patch(Circle((x,y),CONFIG['arrive_r'],ec='blue',ls='--',fc='none',
    ...
    info_txt=ax.text(0.02,0.98,'',transform=ax.transAxes,ha='left',va='top',
                     bbox=dict(fc='white',alpha=0.7),fontsize=9)
    ax.legend(loc='upper right')
    fig.canvas.mpl_connect('key_press_event', lambda e: globals().update(stop_flag=True) if e.key in ['q','escape'] else None)
    fig.canvas.draw()
...
def handle_track(cx,cy,now,wx,wy,st,ser,logs):
    ...
    # 2) arrive_r 벗어날 때만 재진입
    if d2>in2:
        st['target']=find_rejoin_wp(cx,cy,wx,wy,st['yaw'],st['delta'])
        st['wp_reached']=False
    tx,ty=wx[st['target']],wy[st['target']]
...
def update_visualization(cx,cy,tx,ty,fig_ax,route,st):
    ...
    gps_pt.set_data([cx],[cy]); wp_pt.set_data([tx],[ty])
    route_ln.set_data(route[0],route[1]); tgt_ln.set_data([cx,tx],[cy,ty])
    ...
    m=10; ax.set_xlim(cx-m,cx+m); ax.set_ylim(cy-m,cy+m)
...
def run_loop():
    ...
    st = {'state':'INIT','first':False,'prev_x':None,'prev_y':None,'prev_t':None,
          'yaw':0,'v':0,'delta':0,'target':0,'wp_reached':False,
          'last_tx':0,'lat':None,'lon':None,'last_wp':1,'t':time.time()}
    ...
    while not stop_flag:
        if ser_gnss and ser_gnss.in_waiting:
            ...
            now=time.time(); route[0].append(cx);route[1].append(cy)

            if st['state']=='INIT' and not st['first']:
                logs.append([now,st['lat'],st['lon'],0.0,0.0,1])

            if st['state']=='INIT':
                handle_init(cx,cy,now,st,ser_ardu); tx,ty=wx[0],wy[0]
            else:
    ...
    save_logs(logs); print('[INFO] Loop end')

...
# -*- coding: utf-8 -*-
"""
waypoints_live_gngga_viewer_seq_tx.py  (로그 저장 + 경로 이탈 복귀 알고리즘 추가)
• INIT→TRACK 상태머신 + Pure-Pursuit + 이중 반경 히스테리시스
• 매 프레임 로그를 run_loop에서 수집하여 종료 시 CSV로 저장
• 경로 밖(exit_r)으로 벗어날 때, 조향 각도에 맞춰 가장 적절한 웨이포인트로 재진입
"""
...
# ────────────────────────────────────────────────────────────────
CONFIG = {
    'csv_path': r"C:\Users\dlwlr\OneDrive - 한라대학교\바탕 화면\GPS_purse_Python\bocsa\left_lane.csv",
    'gnss_port': 'COM10',
    'ardu_port': 'COM7',
    'baud': 115200,
    'timeout': 0.0,
...
def find_rejoin_wp(cx, cy, wx, wy, yaw, delta):
    """
    exit_r 밖으로 벗어났을 때, 조향 방향(yaw+δ)에 있는
    웨이포인트 중 인덱스가 가장 큰 것을 반환
    """
...
def handle_track(cx, cy, now, wx, wy, st, ser_ardu, logs):
    ...
    st['delta'] = max(-8, min(8,
                      math.degrees(math.atan2(2*CONFIG['wheelbase']*math.sin(alpha), Ld))))

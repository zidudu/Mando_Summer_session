...
#     # Figure 2: Line vs Circle deviation per sample
#     if dev_line or dev_circle:
#         plt.figure(figsize=(8,4))
#         if dev_line:
#             dl = np.concatenate(dev_line)
#             plt.plot(dl, 'g-o', label='Line Dev', markersize=3)
#         if dev_circle:
#             dc = np.concatenate(dev_circle)
#             plt.plot(dc, 'r-s', label='Circle Dev', markersize=3)
#         plt.xlabel('Sample Index'); plt.ylabel('Deviation [m]')
#         plt.title('Deviation vs Sample Index (Line vs Circle)')
#         plt.legend(); plt.grid(True); plt.tight_layout(); plt.show()
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
arc_spline_visualizer.py  (UPDATED with Curvature Profile)

• 왼쪽   : GPS 원시 궤적 - 모델(직선·원·클로소이드) 오버레이
• 가운데 : 편차 히스토그램(Count)
• 오른쪽 : 요약 통계 막대그래프(Mean·Std·RMSE·Min·Max·Range) – 소수점 6자리 표기
• 아래   : 샘플 인덱스별 직선/원호 편차 막대그래프
• 추가   : 곡률 프로파일 그래프 & 세그먼트 분류 출력
"""
...
IN_CSV            = "raw_track_xy_6.csv"
# 곡률 임계값: 실험적으로 조정하세요
LINE_CURV_THR     = 0.02   # κ < 0.02 → 직선
CIRCLE_CURV_THR   = 0.05   # 0.02 ≤ κ < 0.05 → 원호
MIN_SEG_LEN       = 5      # 최소 세그먼트 길이
SMOOTH_S          = 0.5    # 스플라인 스무딩 파라미터
# ────────────────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    plt.plot(kappa, 'o-', label='curvature κ')
    plt.axhline(LINE_CURV_THR,   color='g', linestyle='--', label='LINE_CURV_THR')
    plt.axhline(CIRCLE_CURV_THR, color='r', linestyle='--', label='CIRCLE_CURV_THR')
    plt.xlabel('Sample Index'); plt.ylabel('κ [1/m]')
    plt.title('Curvature Profile'); plt.legend(); plt.grid(True)
    plt.tight_layout(); plt.show()

    # 3) 세그먼트 분류
    segs, types = segment_by_curv(kappa)
    print('Segments:', list(zip(types, segs)))
    ...
    for (i0,i1), tp in zip(segs, types):
        ...
        if tp=='line':
            ...
            dev, _ = fit_clothoid_spline(sx, sy)
            dev_clo.append(dev)

    # 5) Figure 1: Track/Fits + Histogram + Stats Bar
    fig, (ax_trk, ax_hist, ax_bar) = plt.subplots(1,3, figsize=(15,5))
    ...
    for (i0,i1), tp in zip(segs, types):
        sx, sy = xs[i0:i1], ys[i0:i1]
        if tp=='line':
            _, m, b = fit_line(sx, sy)
            ax_trk.plot([sx[0],sx[-1]], [m*sx[0]+b, m*sx[-1]+b], 'g--', label='Line' if 'Line' not in ax_trk.get_legend_handles_labels()[1] else '')
        elif tp=='circle':
            _, xc, yc, r = fit_circle(sx, sy)
            th = np.linspace(0,2*np.pi,200)
            ax_trk.plot(xc+r*np.cos(th), yc+r*np.sin(th), 'r-', label='Circle' if 'Circle' not in ax_trk.get_legend_handles_labels()[1] else '')
        else:
            _, (mode,obj) = fit_clothoid_spline(sx, sy)
            if mode=='clothoid':
                s = np.linspace(0,obj.L,200)
                ax_trk.plot(obj.X(s), obj.Y(s), 'b-', label='Clothoid' if 'Clothoid' not in ax_trk.get_legend_handles_labels()[1] else '')
            else:
                ...
                xs_s, ys_s = splev(u, obj)
                ax_trk.plot(xs_s, ys_s, 'b--', label='Spline' if 'Spline' not in ax_trk.get_legend_handles_labels()[1] else '')
    ax_trk.set_title('Track & Fits'); ax_trk.axis('equal'); ax_trk.grid(True); ax_trk.legend()

    # 5-B Histogram
    all_dev = np.concatenate(dev_line + dev_circle + dev_clo)
    ax_hist.hist(all_dev, bins=30, color='tab:purple', alpha=0.7)
    ax_hist.set_title('Deviation Distribution'); ax_hist.set_xlabel('Deviation [m]'); ax_hist.set_ylabel('Count'); ax_hist.grid(axis='y', linestyle='--', alpha=0.6)

    # 5-C Stats Bar
    groups, lbls = [], []
    if dev_line:   groups.append(stats(np.concatenate(dev_line)));   lbls.append('Line')
    if dev_circle: groups.append(stats(np.concatenate(dev_circle))); lbls.append('Circle')
    if dev_clo:    groups.append(stats(np.concatenate(dev_clo)));    lbls.append('Clothoid')
    if groups:
        groups = np.vstack(groups)
        metrics = ['mean','std','rmse','min','max','range']
        x = np.arange(len(metrics)); w = 0.8/len(lbls)
        for idx,(lbl,row) in enumerate(zip(lbls, groups)):
            ...
            for xi,val in zip(x+idx*w, row):
                ax_bar.text(xi, val+0.0005, f"{val:.6f}", ha='center', va='bottom', fontsize=7)
        ax_bar.set_xticks(x + w*(len(lbls)-1)/2); ax_bar.set_xticklabels(metrics)
        ax_bar.set_title('Error Statistics'); ax_bar.set_ylabel('Deviation [m]'); ax_bar.grid(axis='y', linestyle='--', alpha=0.6); ax_bar.legend()
    else:
        ax_bar.text(0.5,0.5,'No segments',ha='center',va='center'); ax_bar.axis('off')

    plt.tight_layout(); plt.show()

    # 6) Figure 2: Bar charts for Line & Circle deviations
    if dev_line or dev_circle:
        ...
        if dev_line:
            ...
            axs2[0].set_xlabel('Sample Index'); axs2[0].set_ylabel('Deviation [m]'); axs2[0].grid(True)
        else:
            axs2[0].text(0.5,0.5,'No line segments',ha='center',va='center'); axs2[0].axis('off')
        # 곡선
        if dev_circle:
            dc = np.concatenate(dev_circle)
            axs2[1].bar(np.arange(len(dc)), dc, color='b')
            axs2[1].set_title('Circle Deviation per Sample')
            axs2[1].set_xlabel('Sample Index'); axs2[1].set_ylabel('Deviation [m]'); axs2[1].grid(True)
        else:
            axs2[1].text(0.5,0.5,'No circle segments',ha='center',va='center'); axs2[1].axis('off')
        plt.tight_layout(); plt.show()

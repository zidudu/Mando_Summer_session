...
TOPIC_WP_INDEX     = '/tracker/wp_index'      # Int32 (기본 1-based)
...
rtk_status_txt = "NONE"
last_fix_time  = 0.0
wp_index_active = 0      # 퍼블리시용 현재 타겟 인덱스(0-based 내부/퍼블리시는 1-based 기본)
...
def _default_paths():
    ...
    # 기존 내 코드처럼 패키지 내부 경로 사용 (scripts/left_lane.csv 디폴트)
    waypoint_csv = os.path.join(pkg_path, 'scripts', 'left_lane.csv')
...
def _cb_relpos(msg):
    # UBX NAV-RELPOSNED flags.carrSoln: 0=NONE, 1=FLOAT, 2=FIXED (u-blox 문서 기준)
    # https://docs에서 스크린샷 근거 참고
    global rtk_status_txt
    try:
        carr_soln = int((int(msg.flags) >> 3) & 0x3)  # NavRELPOSNED9 기준 carrSoln bit 위치(메시지 정의에 따라 다를 수 있음)
        if carr_soln == 2:
            rtk_status_txt = "FIX"
        elif carr_soln == 1:
            rtk_status_txt = "FLOAT"
        else:
            rtk_status_txt = "NONE"
    except Exception:
        rtk_status_txt = "NONE"
...
def publish_all(event, speed_code_default=1, one_based=True):
    ...
    # 속도: 코드 0~4, 기본 1/2 고정 사용
    code = int(rospy.get_param('~speed_code', speed_code_default))
    code_cap = int(rospy.get_param('~speed_cap_code', 4))
    code = max(0, min(code, code_cap))
...
def main():
    ...
    rospy.on_shutdown(_on_shutdown)  # 종료 훅 등록 (안전정지)  # ref: rospy.on_shutdown 행동 설명 :contentReference[oaicite:2]{index=2}
    ...
    gps_to       = float(rospy.get_param('~gps_timeout_sec', GPS_TIMEOUT_SEC))
    ...
    # CSV 로드 (내 코드 방식: 패키지경로 기본 + 파라미터로 덮기)
    ...
    if ANNOTATE_WAYPOINT_INDEX or DRAW_WAYPOINT_CIRCLES:
        for idx, (x, y) in enumerate(zip(spaced_x, spaced_y), 1):
            ...
            if DRAW_WAYPOINT_CIRCLES:
                ax.add_patch(Circle((x, y), TARGET_RADIUS_END, color='blue', fill=False, linestyle='--', alpha=0.5))
        ...
        while not rospy.is_shutdown():
            ...
            while not gps_queue.empty():
                ...
                rospy.loginfo_throttle(0.5,
                    f"WP {wp_index_active+1}/{len(spaced_x)} | v={speed_mps:.2f} m/s | "

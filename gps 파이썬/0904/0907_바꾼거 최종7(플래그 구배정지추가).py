...
# ── 플래그(구간) 정의 ─────────────────────────────────────────────
# start/end는 "플롯에 보이는 1-based 인덱스"로 적기
# radius_scale: 이 구간에서 WP 판정 반경 배율(0.5면 절반)
# lookahead_scale: 이 구간에서 룩어헤드 배율(코너 보수화)
# stop_on_hit: '해당 구간 안의 WP 원에 닿는 순간' 0,0 퍼블리시로 정지
# stop_duration_sec: 정지 유지 시간
FLAG_DEFS = [
    {'name':'GRADE_UP', 'start':12, 'end':18, 'radius_scale':0.6, 'lookahead_scale':0.9,
     'stop_on_hit': True, 'stop_duration_sec': 3.0},
    # 예시들 (원하면 추가)
    # {'name':'S_CURVE', 'start':25, 'end':34, 'radius_scale':0.8, 'lookahead_scale':0.9},
    # {'name':'RIGHT_ANGLE', 'start':41, 'end':42, 'radius_scale':0.6, 'lookahead_scale':0.7},
]
...
# 플래그/구간 상태
FLAG_ZONES = []         # 정규화된 구간 리스트
current_zone = None     # 활성 구간(dict) 또는 None
prev_zone_name = None
zone_armed = True       # 구간 내 "WP 원 진입 트리거" 무장 상태(원 밖으로 나갔다 다시 들어올 때 재무장)
hold_active = False     # 0,0 퍼블리시 중인지
hold_until  = 0.0       # hold 종료 시각
hold_reason = ""        # 예: "GRADE_UP"
last_hold_wp_idx = 0    # 마지막으로 hold 유발한 1-based WP
...
# 플래그 유틸/훅
# ──────────────────────────────────────────────────────────────────
def _normalize_flags(flag_defs):
    out = []
    for z in flag_defs:
        s = int(z['start']); e = int(z['end'])
        if s > e: s, e = e, s
        out.append({
            'name': str(z['name']),
            'start': s, 'end': e,                   # 1-based inclusive
            'radius_scale': float(z.get('radius_scale', 1.0)),
            'lookahead_scale': float(z.get('lookahead_scale', 1.0)),
            'stop_on_hit': bool(z.get('stop_on_hit', False)),
            'stop_duration_sec': float(z.get('stop_duration_sec', 0.0)),
            'disp_range': f"{s}~{e}" if s!=e else f"{s}"
        })
    return out

def _zone_for_index(idx1):
    """idx1: 1-based 인덱스 → 해당되는 첫 구간 리턴 or None"""
    for z in FLAG_ZONES:
        if z['start'] <= idx1 <= z['end']:
            return z
    return None

def on_enter_zone(zone):
    rospy.loginfo(f"[FLAG] ENTER {zone['name']} [{zone['disp_range']}]")

def on_exit_zone(zone):
    rospy.loginfo(f"[FLAG] EXIT  {zone['name']} [{zone['disp_range']}]")
...
def main():
    ...
    global _last_log_wall, last_pub_speed_code
    global FLAG_ZONES, current_zone, prev_zone_name, zone_armed
    global hold_active, hold_until, hold_reason, last_hold_wp_idx
    ...
    # 플래그 정규화
    FLAG_ZONES = _normalize_flags(FLAG_DEFS)
    ...
    try:
        while not rospy.is_shutdown():
            ...
            while not gps_queue.empty():
                ...
                # 현재 타겟(1-based)
                tgt_idx = target_index_from_lookahead(nearest_idx, max(LOOKAHEAD_MIN, prev_Ld), WAYPOINT_SPACING, len(spaced_x))
                tgt_idx1 = tgt_idx + 1

                # 활성 구간(플래그) 판별 (타겟 인덱스 기준)
                zone_now = _zone_for_index(tgt_idx1)

                # 구간 전환시 훅
                if (current_zone is None and zone_now is not None) or \
                   (current_zone is not None and (zone_now is None or zone_now['name'] != current_zone['name'])):
                    if current_zone is not None:
                        on_exit_zone(current_zone)
                    if zone_now is not None:
                        on_enter_zone(zone_now)
                        zone_armed = True   # 새 구간 진입 시 트리거 재무장
                    current_zone = zone_now

                # 룩어헤드 타겟/스무딩 (구간 배율 반영)
                look_scale = current_zone['lookahead_scale'] if current_zone else 1.0
                Ld_target = max(LOOKAHEAD_MIN, min(LOOKAHEAD_MAX, LOOKAHEAD_MIN + LOOKAHEAD_K * speed_mps))
                Ld_target *= look_scale
                ...
                # 퍼블리시/표시용 인덱스: 반경 안이면 0-based, 아니면 -1 (구간별 반경 배율 적용)
                rad_scale = current_zone['radius_scale'] if current_zone else 1.0
                eff_radius = TARGET_RADIUS_END * rad_scale
                wp_index_active = nearest_in_radius_index(x, y, spaced_x, spaced_y, eff_radius)

                # ── 플래그: stop_on_hit 테스트(언덕) ─────────────────
                # 구간 내이고, 구간이 stop_on_hit=True이며,
                # "WP 원에 처음 진입"하는 순간 hold 시작 (원 밖→안으로 전이 감지용 zone_armed 사용)
                if current_zone and current_zone.get('stop_on_hit', False):
                    wp_display = (wp_index_active + 1) if (wp_index_active >= 0) else 0
                    in_zone_wp = (wp_display >= current_zone['start'] and wp_display <= current_zone['end'])
                    if in_zone_wp:
                        if zone_armed and not hold_active:
                            hold_active = True
                            hold_until = time.time() + float(current_zone.get('stop_duration_sec', 3.0))
                            hold_reason = current_zone['name']
                            last_hold_wp_idx = wp_display
                            zone_armed = False  # 같은 원에서 중복 트리거 방지
                            rospy.loginfo(f"[FLAG] HOLD start ({hold_reason}) at WP {last_hold_wp_idx} for {current_zone.get('stop_duration_sec',3.0)}s")
                    else:
                        # 원 밖으로 나갔으면 재무장
                        zone_armed = True
                ...
                wp_display = (wp_index_active + 1) if (wp_index_active >= 0) else 0
                zone_txt = f"{current_zone['name']}[{current_zone['disp_range']}]" if current_zone else "NONE"
                ...
                ax_info.text(0.02, 0.55, " | ".join([s for s in status if s]), fontsize=11, va='center')

                # 메인 플롯 HUD(오른쪽 아래)
                hud_text.set_text(
                    f"pub_v={last_pub_speed_code:.1f} | meas_v={speed_mps:.2f} m/s | "
                    f"steer={latest_filtered_angle:+.1f}° | WP={wp_display} | FLAG={zone_txt} {'| '+hold_txt if hold_txt else ''}"
                )
                        ...
                        with open(log_csv_path, 'a', newline='') as f:
                            ...
                            if new:
                                w.writerow(['time','lat','lon','x','y',
                                            ...
                                            'tx','ty',
                                            'steer_deg','meas_speed_mps','pub_v_code','Ld',
                                            'flag','hold','rtk'])
                            w.writerow([time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
                                        ...
                                        f"{last_pub_speed_code:.1f}", f"{Ld:.2f}",
                                        zone_txt, hold_txt, rtk_status_txt])
                        _last_log_wall = noww
                ...
                rospy.loginfo_throttle(
                    0.5,
                    f"FLAG={zone_txt} {hold_txt} | WP(in-radius)={wp_display} / tgt={tgt_idx+1}/{len(spaced_x)} | "

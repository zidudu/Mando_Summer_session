...
MIN_WAYPOINT_DISTANCE_DEFAULT = 1.5
...
# (A) 시작 워밍업
START_WARMUP_SEC_DEFAULT = 1.0     # [s]
...
# (E) 반경 히스테리시스
RADIUS_HIT_COUNT_DEFAULT = 6       # 연속 N회 반경 내에 들어와야 인덱스 증가 (0904 2에서 6)
...
def _cb_fix(msg: NavSatFix):
    ...
    global _head_buf, _last_fix_walltime
...
def update_plot_once(ax):
    ...
    with _state_lock:
        ...
        # ✅ GPS 타임아웃 판단을 위한 age 계산
        last_fix_age = (time.time() - _last_fix_walltime) if _last_fix_walltime else 1e9

    # ── ✅ GPS 타임아웃: 즉시 0,0 퍼블리시 후 반환 ──
    if last_fix_age > float(params['gps_timeout_sec']):
        publish_speed_cmd(0.0)
        publish_steer_deg(0.0)
        # 간단한 화면 표기만 남기고 제어 생략
        ax.set_title(f"ROS GPS Tracker  [GPS TIMEOUT {last_fix_age:.2f}s]  → speed=0, steer=0")
...
def main():
    ...
    params = {
        ...
        # ✅ GPS 타임아웃(신규)
        'gps_timeout_sec':    float(rospy.get_param('~gps_timeout_sec', GPS_TIMEOUT_SEC_DEFAULT)),
    }

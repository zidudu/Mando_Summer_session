...
#  - radius_scale, lookahead_scale로 구간 내 동작 튜닝
#    (예: 구배 구간에서 반경을 0.5배로 줄여 정밀정지)
# ──────────────────────────────────────────────────────────────────
FLAG_DEFS = [
    # 예시) 실제 사용하는 구간은 아래 주석 해제하고 숫자만 채워서 쓰면 됨
    # {'name': 'GRADE_UP',   'start': 10, 'end': 15, 'radius_scale': 0.5, 'lookahead_scale': 0.8},
    # {'name': 'S_CURVE',    'start': 20, 'end': 28, 'radius_scale': 0.8, 'lookahead_scale': 0.9},
    # {'name': 'RIGHT_ANGLE','start': 35, 'end': 36, 'radius_scale': 0.6, 'lookahead_scale': 0.7},
    # {'name': 'T_JUNCTION', 'start': 50, 'end': 55, 'radius_scale': 0.7, 'lookahead_scale': 1.0},
    # {'name': 'STRAIGHT_AVOID','start': 70, 'end': 80, 'radius_scale': 1.0, 'lookahead_scale': 1.0},
]

# (선택) 구간 진입/이탈시 훅 함수(필요 없으면 비워두어도 됨)
def on_enter_grade_up(zone):   rospy.loginfo(f"[flag] ENTER GRADE_UP {zone['disp_range']}")
def on_exit_grade_up(zone):    rospy.loginfo(f"[flag] EXIT  GRADE_UP {zone['disp_range']}")
def on_enter_s_curve(zone):    rospy.loginfo(f"[flag] ENTER S_CURVE {zone['disp_range']}")
def on_exit_s_curve(zone):     rospy.loginfo(f"[flag] EXIT  S_CURVE {zone['disp_range']}")
def on_enter_right_angle(zone):rospy.loginfo(f"[flag] ENTER RIGHT_ANGLE {zone['disp_range']}")
def on_exit_right_angle(zone): rospy.loginfo(f"[flag] EXIT  RIGHT_ANGLE {zone['disp_range']}")
def on_enter_t_junction(zone): rospy.loginfo(f"[flag] ENTER T_JUNCTION {zone['disp_range']} (예: 초음파 우선)")
def on_exit_t_junction(zone):  rospy.loginfo(f"[flag] EXIT  T_JUNCTION {zone['disp_range']} (예: GPS 우선)")
def on_enter_straight_avoid(zone): rospy.loginfo(f"[flag] ENTER STRAIGHT_AVOID {zone['disp_range']}")
def on_exit_straight_avoid(zone):  rospy.loginfo(f"[flag] EXIT  STRAIGHT_AVOID {zone['disp_range']}")

FLAG_HOOKS = {
    'GRADE_UP':       (on_enter_grade_up,       on_exit_grade_up),
    'S_CURVE':        (on_enter_s_curve,        on_exit_s_curve),
    'RIGHT_ANGLE':    (on_enter_right_angle,    on_exit_right_angle),
    'T_JUNCTION':     (on_enter_t_junction,     on_exit_t_junction),
    'STRAIGHT_AVOID': (on_enter_straight_avoid, on_exit_straight_avoid),
}
...
active_flag = None           # {'name', 'start0', 'end0', 'radius_scale', 'lookahead_scale', 'disp_range'}
just_entered = False
just_exited = False
...
def flag_update_state(x, y, nearest_idx, xs, ys, base_radius):
    """
    리턴:
      eff_radius, lookahead_scale, active_name, entered(bool), exited(bool)
    """
    ...
    # 1) 진입 체크(아무 플래그도 없는 상태에서 '시작WP 원'에 들어오면 진입)
    ...
    # 2) 활성 중이면 종료 체크: end0를 지나쳤거나 충분히 멀어졌으면 종료
    else:
        z = active_flag
        # 종료 조건(간단): 최근접 인덱스가 end0보다 커지면(구간 통과)
    ...
    return eff_radius, look_scl, active_name, just_entered, just_exited
    ...
    try:
        while not rospy.is_shutdown():
            ...
            while not gps_queue.empty():
                ...
                # ── 플래그 상태 갱신(반경/룩어헤드 스케일 결정) ──
                eff_radius, look_scl, active_name, entered, exited = flag_update_state(
                ...
                # 이벤트 메시지
                if entered and active_name:
                    status.append(f"[ENTER {active_name}]")
                if exited:
                    status.append("[EXIT FLAG]")

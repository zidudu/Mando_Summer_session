...
# ──────────────────────────────────────────────────────────────────
# 복귀(Off-track Recovery) 파라미터
# ──────────────────────────────────────────────────────────────────
OFFTRACK_TRIGGER_MULT = 4.0   # 활성 타깃까지 거리가 반경의 몇 배면 '이탈'로 볼지
RECOVERY_HIT_N        = 2     # 반경 내 연속 진입 횟수(복귀 완료 판정)
RECOVERY_HEADING_DEG  = 30.0  # 복귀 타깃 후보를 고를 때 허용 헤딩차(도)
RECOVERY_SEARCH_AHEAD = 30    # 앞쪽 후보 범위(인덱스 개수)
RECOVERY_LD_CAP       = 2.0   # (선택) 복귀 중 lookahead 상한(m), 사용하려면 주석 해제
...
# ── 복귀 타깃 선택(헤딩/거리 기반) ─────────────────────────
def pick_recovery_target(cx, cy, heading_vec, start_idx, spaced_x=None, spaced_y=None):
    """현재 위치/헤딩 기준으로 앞쪽 후보 중 헤딩 차가 작은 점 1개를 선택"""
    if heading_vec is None:
        return None
    if spaced_x is None or spaced_y is None:
        return None
    hv = np.array(heading_vec, float)
    n = np.linalg.norm(hv)
    if n < 1e-9:
        return None
    hv = hv / n
    ...
    end_idx = min(len(spaced_x)-1, start_idx + RECOVERY_SEARCH_AHEAD)
    for i in range(start_idx, end_idx+1):
        tx, ty = spaced_x[i], spaced_y[i]
        dv = np.array([tx - cx, ty - cy], float)
        d  = np.linalg.norm(dv)
        if d < 1e-6:
            return i
        dvn = dv / d
        cosang = np.clip(hv.dot(dvn), -1.0, 1.0)
        ang = math.degrees(math.acos(cosang))
        if ang <= RECOVERY_HEADING_DEG:
            cost = d + 0.1 * ang  # 거리 우선 + 각도 페널티
            if cost < best_cost:
...
def main():
    ...
    global in_recovery, recovery_target_idx, recovery_hit_cnt
    ...
    try:
        while not rospy.is_shutdown():
            ...
            while not gps_queue.empty():
                ...
                # 동적 Lookahead (복귀 중에는 보수적으로 제한 가능)
                ...
                # 복귀 모드: 타깃 잠금 + 히스테리시스
                # ──────────────────────────────────────────────
                # 활성 타깃 선택 (복귀 중이면 잠금 타깃 우선)
                active_idx = recovery_target_idx if (in_recovery and recovery_target_idx is not None) else tgt_idx
                tx, ty = spaced_x[active_idx], spaced_y[active_idx]

                # 활성 타깃 라인 표시
                target_line.set_data([x, tx], [y, ty])

                # 활성 타깃까지의 거리
                dist_to_active = math.hypot(tx - x, ty - y)

                # 이탈 감지 → 복귀 진입
                if not in_recovery and dist_to_active > OFFTRACK_TRIGGER_MULT * TARGET_RADIUS_END:
                    cand = pick_recovery_target(x, y, last_heading_vec, nearest_idx, spaced_x, spaced_y)
                    if cand is not None and cand >= nearest_idx:
                        in_recovery = True
                        recovery_target_idx = cand
                        recovery_hit_cnt = 0
                        # (선택) 복귀 동안 lookahead 보수적 제한
                        prev_Ld = min(prev_Ld, RECOVERY_LD_CAP)

                # 복귀 진행/완료 (히스테리시스)
                if in_recovery:
                    if dist_to_active <= TARGET_RADIUS_END:
                        recovery_hit_cnt += 1
                        if recovery_hit_cnt >= RECOVERY_HIT_N:
                            in_recovery = False
                            # 정상 모드 복귀: 순차 추적이 자연스럽게 이어지도록 근접 인덱스 업데이트
                            nearest_idx_prev = max(nearest_idx_prev, recovery_target_idx)
                            recovery_target_idx = None
                            recovery_hit_cnt = 0
                    else:
                        recovery_hit_cnt = 0
                ...
                mode_txt = "RECOVERY" if in_recovery else "NORMAL"
                ...
                status = [
                    ...
                    f"Tgt(active): {active_idx+1}/{len(spaced_x)}",
                ...
                # 메인 플롯 HUD(오른쪽 아래)
                hud_text.set_text(
                    f"{mode_txt} | pub_v={last_pub_speed_code:.1f} | meas_v={speed_mps:.2f} m/s | "
                    f"steer={latest_filtered_angle:+.1f}° | WP={wp_display} | tgt={active_idx+1}"
                )
                        ...
                        with open(log_csv_path, 'a', newline='') as f:
                            ...
                            if new:
                                w.writerow(['time','lat','lon','x','y',
                                            'wp_in_radius(1based)','tgt_idx_lookahead(1based)','tgt_idx_active(1based)',
                                            'tx','ty',
                                            'steer_deg','meas_speed_mps','pub_v_code','Ld','mode','rtk'])
                            w.writerow([time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
                                        f"{lat:.7f}", f"{lon:.7f}", f"{x:.3f}", f"{y:.3f}",
                                        wp_display, tgt_idx+1, active_idx+1, f"{tx:.3f}", f"{ty:.3f}",
                                        f"{latest_filtered_angle:.2f}", f"{speed_mps:.2f}",
                                        f"{last_pub_speed_code:.1f}", f"{Ld:.2f}",
                                        "RECOVERY" if in_recovery else "NORMAL", rtk_status_txt])
                        _last_log_wall = noww
                ...
                rospy.loginfo_throttle(
                    0.5,
                    f"{'REC' if in_recovery else 'NOR'} | WP(in-radius)={wp_display} / "
                    f"tgtL={tgt_idx+1}→act={active_idx+1}/{len(spaced_x)} | "

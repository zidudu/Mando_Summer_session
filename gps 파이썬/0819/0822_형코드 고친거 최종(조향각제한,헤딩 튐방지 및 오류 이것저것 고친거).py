...
안정화 구조: 헤딩은 “최소 이동거리 + 점프각 제한 + 거리 가중 평균”으로 갱신되어, 저속/정지 구간의 노이즈에 비교적 강합니다.

경로 이탈 방지: 전방 윈도우 + 룩어헤드 + ±각도 게이트 조합이 교차로에서의 잘못된 가지 선택을 체계적으로 줄입니다.

안전 여지: RTK 품질(FIX/FLOAT/NONE)에 따라 속도 캡·도착반경을 조정하므로, 수신 품질 저하 시 과도한 추종을 예방합니다.

튜닝 용이성: 대부분이 파라미터화되어 있어 현장·노선별로 재컴파일 없이 조정 가능합니다.

한계/리스크

GNSS 단독 한계: 정지·초저속(≈0.1 m/s 이하)에서는 GNSS만으로 진행방향 정의가 불안정합니다(물리적 한계). 프리즈·롤인으로 완화하지만 완전 대체는 어렵습니다.

교차로 근접 간격: 웨이포인트가 가지끼리 매우 근접하면(수 m 이내 병렬) 전방 윈도우 안에 여러 가지가 동시에 들어와 게이트만으로는 드물게 오판 가능성이 있습니다(차선 코리도어/맵매칭이 있으면 더 견고).

지연(레이턴시): 1차 LPF(2 Hz)로 인한 추가 동특성 지연이 대략 80~100 ms 수준이며, 20 Hz 루프(50 ms)와 합쳐 체감 지연이 생길 수 있습니다(수치는 환경·하드웨어에 따라 다소 변동).

곡률 적응 한계: 조향기반 감속은 단순 선형 규칙이어서 급격한 곡률 변화 구간에서는 최적 속도 계획 대비 보수적이거나 부족할 수 있습니다.

성능/복잡도 관점

계산량: 20 Hz 기준 CPU 부하는 매우 낮은 편(벡터 연산·간단한 필터·인덱스 스캔). 라즈베리 파이급에서도 충분.

스케일링: 웨이포인트 수가 매우 커도 전방 윈도우 스캔이어서 선형이 아니라 상수 시간에 가깝게 동작합니다.

재현성/디버그

로그 품질: 타깃 인덱스·헤딩·조향·RTK 상태·거리 등을 CSV로 남겨 재현·리플레이 분석이 용이합니다.

관측 가능성: 시각화(타깃/헤딩/조향 화살표)가 실시간 의사결정(타깃 선택·복귀)과 일치해 원인 파악이 쉽습니다.

확장/개선 여지(객관적 제안)

IMU 융합: 요레이트/자이로를 융합하면 저속·정지 구간 헤딩 신뢰도가 크게 향상됩니다(EKF/컴플리먼터리 필터).

코리도어 제약: 경로 중심선 대비 횡오차 한계(예: ±2.0~2.5 m)를 추가하면 교차로 오판이 더 줄어듭니다.

곡률 기반 속도계획: 웨이포인트 간 곡률로 목표 속도 프로파일을 만들면 감속이 덜 보수적이면서 흔들림이 감소합니다.

룩어헤드 적응: 현재 곡률·RTK 품질·속도를 함께 반영해 Ld를 적응시키면 코너/직선 전환 품질이 상승합니다.
...
명령 지연(측정: 센서 타임스탬프→퍼블리시까지): ms

RTK 상태별 성능(FIX/FLOAT/NONE 구간 분리): CTE·속도·도달율

요약하면, 현재 구조는 GNSS 단독 조건에서 구현 복잡도 대비 안정성이 좋은 균형점입니다. 다만 초저속/정지·교차로 근접 가지가 핵심 난점이므로, 필요 시 IMU 융합과 코리도어 제약을 추가하시면 객관적 성능이 한 단계 더 올라갈 것입니다.

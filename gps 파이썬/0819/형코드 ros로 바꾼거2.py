...
- 포트 자동탐색:
    ~nmea_port 가 'auto' 이면 /dev/serial/by-id/* → /dev/ttyACM* → /dev/ttyUSB* 순으로
    후보를 프루브(GGA 수신 검사)하여 선택. 중복접속 방지(exclusive) 및 끊김/오류시 재연결.
"""
...
PROBE_TIMEOUT_DEFAULT = 2.0  # 포트 프루브 시간(s)
...
def _serial_candidates():
    ...
    tried = set()
    order = [
        '/dev/serial/by-id/*',  # 가장 안정적 (디바이스 고유 ID)
        '/dev/ttyACM*',
        '/dev/ttyUSB*',
    ]
    out = []
...
def _open_serial(path, baud, timeout, exclusive=True):
    """pyserial 버전에 따라 exclusive 인자 지원/미지원 호환."""
    try:
        return serial.Serial(path, baud, timeout=timeout, exclusive=exclusive)
    except TypeError:
        return serial.Serial(path, baud, timeout=timeout)

def _probe_nmea_port(path, baud, timeout_s=2.0):
    """포트를 잠깐 열어 GxGGA가 나오는지 확인."""
    try:
        ser = _open_serial(path, baud, timeout=0.2, exclusive=True)
    except Exception:
        return False
    try:
        ...
        while (time.time() - t0) < timeout_s:
            try:
                ...
                if s.startswith('$GNGGA') or s.startswith('$GPGGA'):
                    return True
            except Exception:
                pass
        return False
...
def _pick_nmea_port(wanted, baud, timeout_s):
    """wanted='auto'면 후보를 프루브하여 첫 성공 포트를 선택."""
    if wanted and wanted != 'auto':
        return wanted
    for cand in _serial_candidates():
        if _probe_nmea_port(cand, baud, timeout_s):
            rospy.loginfo(f"[tracker_nmea] auto-detected NMEA port: {cand}")
            return cand
    rospy.logwarn("[tracker_nmea] auto-detect failed; will try first candidate if any")
...
def nmea_reader():
    ...
    baud = int(params['nmea_baud'])
    probe_t = float(params.get('probe_timeout', PROBE_TIMEOUT_DEFAULT))

    while not rospy.is_shutdown():
        # 포트 선택(자동/수동)
        port = _pick_nmea_port(params['nmea_port'], baud, probe_t)
        if not port:
            rospy.logerr("[tracker_nmea] no serial candidates found. Re-trying in 2s...")
            time.sleep(2.0)
            continue

        # 오픈
        try:
            ser = _open_serial(port, baud, timeout=0.1, exclusive=True)
            rospy.loginfo(f"[tracker_nmea] serial open: {port}@{baud}")
            ser.reset_input_buffer()
        except Exception as e:
            rospy.logwarn(f"[tracker_nmea] open failed {port}@{baud}: {e}. Re-trying in 2s...")
            time.sleep(2.0)
            continue
            ...
            while not rospy.is_shutdown():
                try:
                    ...
                    if s.startswith('$GNGGA') or s.startswith('$GPGGA'):
                        ...
                        try:
                            q = int(parts[6]) if len(parts) > 6 else 0
                    ...
                    rospy.logwarn(f"[tracker_nmea] serial error: {e}  (will reopen)")
                    break  # 바깥 while로 나가서 재오픈
                except Exception as e:
...
def main():
    ...
    # 파라미터(기본: config 폴더)
    params = {
        'nmea_port':       rospy.get_param('~nmea_port', PORT_DEFAULT),   # 'auto' 지원
        'nmea_baud':       int(rospy.get_param('~nmea_baud', BAUD_DEFAULT)),
        'nmea_probe_timeout': float(rospy.get_param('~nmea_probe_timeout', PROBE_TIMEOUT_DEFAULT)),
        ...
        'probe_timeout':   float(rospy.get_param('~probe_timeout', PROBE_TIMEOUT_DEFAULT)),
    }
    # LPF 계수
